...
const G      = "#2C4A3E";   // Forest green — primary brand, headers, icons
const GOLD   = "#D4A847";   // Amber gold — accents, active states
const CREAM  = "#F7F4EF";   // Cream — page background
const G_MID  = "#4a7a68";   // Mid green — charts, icons
const RED    = "#B94040";   // Risk/critical
const AMBER  = "#C89A2A";   // Caution

// Light theme text scale — all readable on #F7F4EF cream background
const T1 = "#1C2B25";   // Primary text — near black green. WCAG AA+ on cream
const T2 = "#2C4A3E";   // Secondary text — forest green. WCAG AA on cream
const T3 = "#3D5A4F";   // Body text — readable mid green
const T4 = "#5A7A6E";   // Muted — captions, hints
const T5 = "#7A9A8E";   // Subtle — placeholders, dividers text only

// Surface colours
const S1 = "#F7F4EF";   // Page background (cream)
const S2 = "#EEE9E1";   // Card background
const S3 = "#E6E0D8";   // Elevated card
const S4 = "#DDD7CE";   // Input background
const BDR = "rgba(44,74,62,0.2)";  // Default border
...
::-webkit-scrollbar-thumb{background:rgba(212,168,71,0.4);border-radius:3px;}
input,textarea,select{background:#EEE9E1;border:1px solid rgba(44,74,62,0.25);color:#1C2B25;font-family:'DM Sans',sans-serif;font-size:13px;padding:9px 12px;outline:none;width:100%;}
input:focus,textarea:focus,select:focus{border-color:rgba(212,168,71,0.6);box-shadow:0 0 0 2px rgba(212,168,71,0.12);}
select{background:#EEE9E1;cursor:pointer;}
input[type=range]{padding:0;background:transparent;border:none;height:4px;accent-color:#D4A847;}
@keyframes spin{to{transform:rotate(360deg);}}`;

// ─── SHARED COMPONENTS ───────────────────────────────────────────────────────

// Section label — gold mono caps
function SL({children, color=GOLD}){
  return <div style={{...mono,fontSize:10,color,letterSpacing:"0.14em",textTransform:"uppercase",marginBottom:12}}>{children}</div>;
}
...
function Card({children,style={},gold=false}){
  return <div style={{background:S2,border:`1px solid ${gold?"rgba(212,168,71,0.35)":BDR}`,padding:24,...style}}>{children}</div>;
}

// Gold filled button
function GBtn({children,onClick,disabled,style={}}){
  return <button onClick={onClick} disabled={disabled} style={{background:disabled?"rgba(212,168,71,0.3)":GOLD,color:T2,border:"none",padding:"12px 28px",...mono,fontSize:11,letterSpacing:"0.1em",textTransform:"uppercase",cursor:disabled?"not-allowed":"pointer",fontWeight:700,...style}}>{children}</button>;
}
function GoldButton(p){return <GBtn {...p}/>;}

// Outline button — forest green border
function OBtn({children,onClick,style={}}){
  return <button onClick={onClick} style={{background:"transparent",color:T2,border:`1.5px solid ${G}`,padding:"10px 22px",...mono,fontSize:10,letterSpacing:"0.12em",textTransform:"uppercase",cursor:"pointer",...style}}>{children}</button>;
}
function OutlineButton(p){return <OBtn {...p}/>;}

// Callout box — gold tinted
function Callout({children}){
  return <div style={{background:"#F5EED8",border:"1px solid rgba(212,168,71,0.35)",padding:"14px 18px",marginBottom:24,fontSize:13,color:T2,lineHeight:1.7}}>{children}</div>;
}

// Tab bar
function TabBar({tabs,active,onChange}){
  return <div style={{display:"flex",borderBottom:`1px solid ${BDR}`,marginBottom:24,background:S1}}>
    {tabs.map(({id,label})=><button key={id} onClick={()=>onChange(id)} style={{background:"none",border:"none",borderBottom:active===id?`2px solid ${GOLD}`:"2px solid transparent",padding:"11px 18px",...mono,fontSize:10,letterSpacing:"0.12em",textTransform:"uppercase",cursor:"pointer",color:active===id?GOLD:T4,marginBottom:-1,transition:"color 0.15s"}}>{label}</button>)}
  </div>;
}
...
function DataTable({headers,rows}){
  return <div style={{overflowX:"auto"}}>
    <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
      <thead><tr>{headers.map((h,i)=><th key={i} style={{...mono,fontSize:10,color:T4,letterSpacing:"0.08em",textTransform:"uppercase",padding:"8px 12px",textAlign:"left",background:S3,borderBottom:`2px solid ${GOLD}`}}>{h}</th>)}</tr></thead>
      <tbody>{rows.map((row,ri)=><tr key={ri} style={{background:ri%2===0?S2:S1}}>{row.map((cell,ci)=><td key={ci} style={{padding:"9px 12px",color:ci===0?T2:T3,fontWeight:ci===0?600:400,borderBottom:`1px solid ${BDR}`,verticalAlign:"top",lineHeight:1.5}}>{cell}</td>)}</tr>)}</tbody>
    </table>
  </div>;
}
...
const DIAG_DIMS=[
  {id:"strategy_clarity",num:"01",label:"Strategy Clarity",tagline:"Does everyone know where you're going — and why?",
   ...
   levels:[
    {label:"Critical",color:"#B94040",bg:"rgba(185,64,64,0.08)",desc:"No clear strategy exists. Direction shifts constantly.",examples:["No strategy document exists","Ask three leaders what matters — get three answers","Teams executing on last year's plan or gut feel"]},
    {label:"Weak",color:"#C0622A",bg:"rgba(192,98,42,0.08)",desc:"Strategy exists in a deck, not in decisions.",examples:["Presented once, rarely referenced","No narrative for managers to use","Frontline teams can't connect work to strategy"]},
    {label:"Developing",color:"#C89A2A",bg:"rgba(200,154,42,0.08)",desc:"Leaders understand it but translation is uneven.",examples:["Some teams have OKRs linked to strategy","Managers describe it but struggle to apply it","Quarterly reviews reference it but don't drive priorities"]},
    {label:"Capable",color:"#4a7a68",bg:"rgba(74,122,104,0.08)",desc:"Clearly articulated, consistently communicated and cascaded.",examples:["One-page narrative actively used by people leaders","Team goals visibly linked to strategic pillars","Leaders use it to say no and make trade-offs"]},
    {label:"Leading",color:"#2C6B52",bg:"rgba(44,107,82,0.1)",desc:"A living system used as the logic for every decision.",examples:["Reviewed and refreshed quarterly","Every initiative tested against strategic fit","Frontline teams articulate their contribution unprompted"]}
  ]},
  {id:"leadership_alignment",num:"02",label:"Leadership Alignment",tagline:"Are your leaders pulling in the same direction?",
   ...
   levels:[
    {label:"Critical",color:"#B94040",bg:"rgba(185,64,64,0.08)",desc:"Leaders are visibly misaligned. Competing agendas play out publicly.",examples:["Executives sponsor competing initiatives","Decisions made in one forum are relitigated elsewhere","Teams play leaders off against each other"]},
    {label:"Weak",color:"#C0622A",bg:"rgba(192,98,42,0.08)",desc:"Aligned in the room, mixed messages sent outside it.",examples:["Priorities agreed in workshops, silos return at desks","Sponsor visible at launch, absent during delivery","BAU crowds out transformation commitments"]},
    {label:"Developing",color:"#C89A2A",bg:"rgba(200,154,42,0.08)",desc:"Mostly aligned but gaps create friction. Needs constant brokering.",examples:["Agreed priorities but no shared tracking metrics","Some leaders champion change; others are passive","Individual functional priorities still override shared goals"]},
    {label:"Capable",color:"#4a7a68",bg:"rgba(74,122,104,0.08)",desc:"Demonstrably aligned. Consistent messages, shared accountability.",examples:["Shared scorecard with collective transformation accountability","Leaders model the behaviours they ask of others","Sponsor attends key milestones, accessible to the team"]},
    {label:"Leading",color:"#2C6B52",bg:"rgba(44,107,82,0.1)",desc:"Alignment is structural, not relational. Built to outlast personnel changes.",examples:["Performance frameworks explicitly link to transformation delivery","Leadership coalition formally constituted","Leaders advocate for the programme in board reporting"]}
  ]},
  {id:"initiative_prioritisation",num:"03",label:"Initiative Prioritisation",tagline:"Are you doing fewer things, better?",
   intro:"Organisations that fail to ruthlessly prioritise dilute focus, exhaust capacity, and deliver nothing well. The ability to stop work is as important as starting it.",
   levels:[
    {label:"Critical",color:"#B94040",bg:"rgba(185,64,64,0.08)",desc:"No meaningful prioritisation. Everything is urgent, nothing is resourced.",examples:["20+ 'strategic priorities' active simultaneously","New initiatives added without removing existing ones","No process for evaluating work against strategic value"]},
    {label:"Weak",color:"#C0622A",bg:"rgba(192,98,42,0.08)",desc:"Prioritisation is reactive. Whoever asks loudest wins.",examples:["Priority determined by relationships, not merit","Teams re-prioritise constantly on ad hoc requests","No visibility of total initiative load across the org"]},
    {label:"Developing",color:"#C89A2A",bg:"rgba(200,154,42,0.08)",desc:"A framework exists but is applied inconsistently.",examples:["Portfolio register exists but isn't actively managed","High-priority work shares resources equally with BAU","Effort-vs-impact mapping done but not operationalised"]},
    {label:"Capable",color:"#4a7a68",bg:"rgba(74,122,104,0.08)",desc:"Initiatives prioritised against clear criteria. Resources follow priority.",examples:["Portfolio governance determines what proceeds","New intake requires strategic alignment sign-off","Portfolio health reviewed monthly at leadership level"]},
    {label:"Leading",color:"#2C6B52",bg:"rgba(44,107,82,0.1)",desc:"Portfolio discipline embedded in the operating rhythm. Stopping is valued.",examples:["Annual planning includes explicit 'stop doing' decisions","Capacity treated as a strategic asset with defined limits","Benefits realisation informs future prioritisation"]}
  ]},
  {id:"governance_maturity",num:"04",label:"Governance Maturity",tagline:"Are the right decisions being made, by the right people, at the right time?",
   intro:"PMI research consistently links governance maturity to on-time, on-budget delivery. Mature governance is not more bureaucracy — it is faster, cleaner decisions.",
   levels:[
    {label:"Critical",color:"#B94040",bg:"rgba(185,64,64,0.08)",desc:"No governance structure exists. No one accountable for outcomes.",examples:["No steering committee or programme board","Decisions require individual access to busy executives","RACI has never been defined for key workstreams"]},
    {label:"Weak",color:"#C0622A",bg:"rgba(192,98,42,0.08)",desc:"Structures exist on paper but don't function.",examples:["Steering committee meets but decisions aren't documented","Escalation paths routinely worked around","Governance meetings used to report, not decide"]},
    {label:"Developing",color:"#C89A2A",bg:"rgba(200,154,42,0.08)",desc:"Functional but inconsistent. Oversight varies across workstreams.",examples:["Steering committee meets with a structured agenda","Decision logs exist, though follow-through is uneven","Escalation threshold unclear — issues sit too long"]},
    {label:"Capable",color:"#4a7a68",bg:"rgba(74,122,104,0.08)",desc:"Consistently applied. Decision rights clear, issues resolved within SLAs.",examples:["Clear terms of reference and live decision log","RACI actively referenced for significant decisions","Programme board reviews risk register and resourcing monthly"]},
    {label:"Leading",color:"#2C6B52",bg:"rgba(44,107,82,0.1)",desc:"Adaptive and lightweight. Right-sized, reviewed, and trusted.",examples:["Cadence reviewed at each phase gate","Decision authority clearly delegated — not everything reaches the top","Governance health measured and reported as a programme metric"]}
  ]},
  {id:"change_capability",num:"05",label:"Change Capability",tagline:"Can your organisation manage the human side of transformation?",
   intro:"Organisations with excellent change management are 6× more likely to meet project objectives (Prosci). Yet most treat change as a comms plan. That gap is where transformations quietly die.",
   levels:[
    {label:"Critical",color:"#B94040",bg:"rgba(185,64,64,0.08)",desc:"Change management doesn't exist. Resistance managed through mandate.",examples:["No dedicated change resource on any programme","Communication is a single email or town hall","Resistance treated as insubordination, not information"]},
    {label:"Weak",color:"#C0622A",bg:"rgba(192,98,42,0.08)",desc:"Change confused with comms or training. Activity without structure.",examples:["Comms plan exists but no change strategy","Training scheduled at go-live, not sequenced for readiness","No measurement of adoption or resistance"]},
    {label:"Developing",color:"#C89A2A",bg:"rgba(200,154,42,0.08)",desc:"Recognised as a discipline on major programmes. Inconsistent quality.",examples:["ADKAR or Kotter used by some practitioners","Impact assessments conducted but not always actioned","Capability sits with individuals, not embedded in the org"]},
    {label:"Capable",color:"#4a7a68",bg:"rgba(74,122,104,0.08)",desc:"Common methodology applied. Adoption measured. Leaders equipped.",examples:["Dedicated change lead on all significant programmes from initiation","Readiness assessed at multiple milestones using ADKAR","Go-live decisions consider readiness, not just technical completion"]},
    {label:"Leading",color:"#2C6B52",bg:"rgba(44,107,82,0.1)",desc:"Change is an organisational competency, not a project role.",examples:["Change centre of excellence with defined standards","ROI tracked: adoption rates, benefit realisation, productivity recovery","Change maturity reviewed annually"]}
  ]},
  {id:"delivery_discipline",num:"06",label:"Delivery Discipline",tagline:"Do your projects actually deliver what they promised?",
   intro:"PMI research shows high-maturity organisations waste 13× less money than low-maturity peers. Benefits realisation — not go-live — is the real finish line.",
   levels:[
    {label:"Critical",color:"#B94040",bg:"rgba(185,64,64,0.08)",desc:"No structured delivery framework. No visibility of what's at risk.",examples:["No milestone tracking or status reporting","Issues surface at crisis point, not proactively","Projects 'done' when the system goes live — regardless of adoption"]},
    {label:"Weak",color:"#C0622A",bg:"rgba(192,98,42,0.08)",desc:"Basic structures exist but don't drive decisions. Deadlines slip.",examples:["Status reports produced but not used in forums","RAG ratings inflated — green turns red overnight","Project managers are administrators, not delivery leaders"]},
    {label:"Developing",color:"#C89A2A",bg:"rgba(200,154,42,0.08)",desc:"Disciplines applied inconsistently. Some projects well-run, others not.",examples:["Methodology exists but compliance varies","Risk registers updated irregularly","Escalation happens but not always through defined paths"]},
    {label:"Capable",color:"#4a7a68",bg:"rgba(74,122,104,0.08)",desc:"Consistent rigour. Status visible, accurate, and trusted.",examples:["Common methodology with defined tollgates across portfolio","Portfolio status dashboard updated weekly","RAG ratings honest — governance responds when amber or red"]},
    {label:"Leading",color:"#2C6B52",bg:"rgba(44,107,82,0.1)",desc:"Delivery discipline is a competitive advantage. The org learns from every project.",examples:["Post-implementation reviews within 90 days, findings actioned","Benefits tracked at 6 and 12 months post go-live","PMO sets standards and coaches project teams"]}
  ]}
];
...
function Tool1Diagnostic(){
  ...
  function buildRadar(){
    ...
    [0.25,0.5,0.75,1].forEach(lv=>{const pts=DIAG_DIMS.map((_,i)=>{const a=Math.PI*2*i/n-Math.PI/2;return`${cx+Math.cos(a)*r*lv},${cy+Math.sin(a)*r*lv}`;}).join(" ");elems.push(<polygon key={lv} points={pts} fill="none" stroke={G} strokeWidth="0.8" strokeOpacity="0.2"/>);});
    DIAG_DIMS.forEach((_,i)=>{const a=Math.PI*2*i/n-Math.PI/2;elems.push(<line key={`ax${i}`} x1={cx} y1={cy} x2={cx+Math.cos(a)*r} y2={cy+Math.sin(a)*r} stroke={G} strokeWidth="0.8" strokeOpacity="0.2"/>);});
  ...
  return(<div>
    ...
    {view==="input"&&(<div>
      ...
      <div style={{marginBottom:24}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}><span style={{...mono,fontSize:10,color:T3}}>Scoring progress</span><strong style={{...mono,fontSize:10,color:GOLD}}>{scored} of 6 scored</strong></div><div style={{height:2,background:S4,borderRadius:1}}><div style={{height:"100%",width:`${(scored/6)*100}%`,background:GOLD,borderRadius:1,transition:"width 0.4s"}}/></div></div>
      {DIAG_DIMS.map(dim=>{const s=scores[dim.id];const col=scoreColor(s);const isOpen=expanded[dim.id];return(<div key={dim.id} style={{background:S2,border:`1px solid ${BDR}`,marginBottom:2,overflow:"hidden"}}>
        <div onClick={()=>setExpanded(p=>({...p,[dim.id]:!p[dim.id]}))} style={{padding:"18px 22px",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",gap:12}}>
          <div style={{display:"flex",gap:14,alignItems:"center",flex:1}}><span style={{...mono,fontSize:10,color:"rgba(212,168,71,0.55)"}}>{dim.num}</span><div><div style={{...serif,fontSize:15,fontWeight:700,color:T2}}>{dim.label}</div><div style={{fontSize:11,color:T1,marginTop:2}}>{dim.tagline}</div></div></div>
          <div style={{display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
            {s?<span style={{...mono,fontSize:9,color:col,background:`${col}18`,border:`1px solid ${col}44`,padding:"2px 8px",textTransform:"uppercase"}}>{s} — {SLABELS[s]}</span>:<span style={{...mono,fontSize:9,color:T1,background:S2,border:`1px solid ${BDR}`,padding:"2px 8px"}}>Not scored</span>}
            <span style={{color:T3,fontSize:10,transform:isOpen?"rotate(180deg)":"none",transition:"transform 0.2s"}}>▼</span>
          </div>
        </div>
        {isOpen&&(<div style={{borderTop:"1px solid rgba(247,244,239,0.08)",padding:"0 22px 20px"}}>
          <div style={{fontSize:12,color:T2,lineHeight:1.7,padding:"14px 0 16px",borderBottom:"1px solid rgba(247,244,239,0.05)",marginBottom:16,fontStyle:"italic"}}>{dim.intro}</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:3}}>
            {dim.levels.map((lvl,i)=>{const isSel=s===i+1;return(<div key={i} onClick={()=>selectScore(dim.id,i+1)} style={{padding:"12px 10px 14px",cursor:"pointer",border:`2px solid ${isSel?lvl.color:"transparent"}`,background:lvl.bg,position:"relative",transition:"border-color 0.15s"}}>
              {isSel&&<div style={{position:"absolute",top:8,right:8,width:14,height:14,borderRadius:"50%",background:lvl.color,display:"flex",alignItems:"center",justifyContent:"center",fontSize:8,color:T1,fontWeight:700}}>✓</div>}
    ...
    {view==="results"&&avg&&(<div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:2,marginBottom:2}}>
        <Card gold style={{padding:28}}><SL>Overall Maturity Score</SL><div style={{display:"flex",alignItems:"baseline",gap:10,marginBottom:8}}><span style={{...serif,fontSize:56,fontWeight:700,color:GOLD,lineHeight:1}}>{avg.toFixed(1)}</span><span style={{...mono,fontSize:10,color:T1,textTransform:"uppercase",letterSpacing:"0.1em",lineHeight:1.7}}>/ 5.0<br/>{ml}</span></div><div style={{...serif,fontSize:14,color:T1,fontStyle:"italic",lineHeight:1.5,maxWidth:260,marginBottom:20}}>{avg<2.5?"Your organisation is at high risk of execution failure.":avg<3.5?"There are meaningful gaps between strategy and delivery.":avg<4.5?"Your foundation is sound — but gaps remain.":"Your organisation is executing with genuine discipline."}</div><div style={{display:"flex",gap:20}}>{[{l:"Critical",c:risks.length,col:RED},{l:"Caution",c:cautions.length,col:AMBER},{l:"Strength",c:strengths.length,col:G_MID}].map(s=><div key={s.l}><div style={{...serif,fontSize:26,fontWeight:700,color:s.col,lineHeight:1}}>{s.c}</div><div style={{...mono,fontSize:9,color:T1,letterSpacing:"0.1em",textTransform:"uppercase",marginTop:3}}>{s.l}</div></div>)}</div></Card>
        <Card style={{padding:28,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}><SL>Dimension Profile</SL><svg width="240" height="240" viewBox="0 0 260 260">{buildRadar()}</svg><div style={{display:"flex",flexWrap:"wrap",gap:"4px 12px",marginTop:8,justifyContent:"center"}}>{DIAG_DIMS.map(d=><div key={d.id} style={{...mono,fontSize:8,color:T3}}><span style={{color:GOLD}}>{d.num}</span> {d.label}</div>)}</div></Card>
      </div>
      <Card style={{padding:24,marginBottom:2}}><SL>Dimension Breakdown</SL>{DIAG_DIMS.map(dim=>{const s=scores[dim.id]||3;const col=scoreColor(s);return(<div key={dim.id} style={{marginBottom:14}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:5,alignItems:"baseline"}}><div style={{display:"flex",gap:8,alignItems:"baseline"}}><span style={{...mono,fontSize:9,color:"rgba(212,168,71,0.4)"}}>{dim.num}</span><span style={{...serif,fontSize:13,color:T2}}>{dim.label}</span></div><span style={{...mono,fontSize:10,color:col}}>{s} — {SLABELS[s]}</span></div><div style={{height:2,background:S4,borderRadius:1}}><div style={{height:"100%",width:`${((s-1)/4)*100}%`,background:col,borderRadius:1,transition:"width 0.8s"}}/></div></div>);})}</Card>
      {risks.length>0&&<div style={{background:"#F5E8E8",border:"1px solid rgba(185,64,64,0.25)",padding:24,marginBottom:2}}><SL color={RED}>⚠ Key Risk Areas</SL>{risks.map(dim=><div key={dim.id} style={{paddingLeft:16,borderLeft:`2px solid ${RED}`,marginBottom:16}}><div style={{...serif,fontSize:14,color:T1,marginBottom:4}}>{dim.label}</div><div style={{fontSize:12,color:T1,lineHeight:1.7}}>{DRISK[dim.id]}</div></div>)}</div>}
      <Card style={{padding:24,marginBottom:2}}><SL>Improvement Roadmap</SL>{[{label:"Immediate — Address First",dims:risks,color:RED,note:"These gaps are actively undermining execution. Act now — before the next major phase begins."},{label:"Short-Term — Strengthen",dims:cautions,color:AMBER,note:"Developing capability. Embed these into your programme plan in the next 90 days."},{label:"Sustain — Protect Strengths",dims:strengths,color:G_MID,note:"You have genuine capability here. Actively protect it — strengths erode under delivery pressure."}].filter(t=>t.dims.length>0).map((tier,ti)=><div key={ti} style={{marginBottom:28}}><div style={{display:"flex",gap:8,alignItems:"center",marginBottom:4}}><div style={{width:6,height:6,borderRadius:"50%",background:tier.color,flexShrink:0}}/><span style={{...mono,fontSize:10,color:tier.color,textTransform:"uppercase",letterSpacing:"0.12em"}}>{tier.label}</span></div><p style={{fontSize:12,color:T1,fontStyle:"italic",margin:"4px 0 12px 14px"}}>{tier.note}</p>{tier.dims.map(dim=><div key={dim.id} style={{marginBottom:12,paddingLeft:14}}><div style={{...serif,fontSize:13,color:T2,marginBottom:5}}>{dim.label}</div>{DROAD[dim.id].map((a,i)=><div key={i} style={{display:"flex",gap:8,alignItems:"flex-start",marginBottom:3}}><span style={{color:GOLD,fontSize:11,marginTop:2,flexShrink:0}}>—</span><span style={{fontSize:12,color:T2,lineHeight:1.6}}>{a}</span></div>)}</div>)}</div>)}</Card>
      <div style={{background:"#F7F2E2",border:"1px solid rgba(212,168,71,0.3)",padding:24,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:16}}><div><div style={{...serif,fontSize:16,color:T1,marginBottom:4}}>Ready to close the gap?</div><div style={{fontSize:12,color:T1}}>Strategy Made Real works with leadership teams to turn strategy into disciplined execution.</div></div><div style={{display:"flex",gap:10,flexWrap:"wrap"}}><GBtn onClick={()=>exportDiagnostic({scores,dims:DIAG_DIMS,slabels:SLABELS,risks,cautions,strengths,roadmapActions:DROAD,riskMsgs:DRISK})}>↓ Download Report</GBtn><OBtn onClick={()=>setView("input")}>← Revise Inputs</OBtn></div></div>
    </div>)}
  </div>);
}
...
function Tool2Prioritisation(){
  ...
  return(<div>
    ...
    {view==="input"&&(<div>
      ...
      <Card style={{padding:22,marginBottom:12}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
          <div style={{gridColumn:"1/-1"}}><div style={{...mono,fontSize:9,color:T1,letterSpacing:"0.09em",textTransform:"uppercase",marginBottom:4}}>Initiative Name</div><input value={fw} onChange={e=>setFw(e.target.value)} placeholder="e.g. ERP Finance Transformation" style={{width:"100%",background:S2,border:`1px solid ${BDR}`,padding:"8px 11px",color:T1,...sans,fontSize:13,outline:"none"}}/></div>
          <div><div style={{...mono,fontSize:9,color:T1,letterSpacing:"0.09em",textTransform:"uppercase",marginBottom:4}}>Owner</div><input value={fo} onChange={e=>setFo(e.target.value)} placeholder="e.g. CFO" style={{width:"100%",background:S2,border:`1px solid ${BDR}`,padding:"8px 11px",color:T1,...sans,fontSize:13,outline:"none"}}/></div>
          <div><div style={{...mono,fontSize:9,color:T1,letterSpacing:"0.09em",textTransform:"uppercase",marginBottom:4}}>Duration</div><select value={fd} onChange={e=>setFd(e.target.value)} style={{width:"100%",background:S1,border:`1px solid ${BDR}`,padding:"8px 11px",color:T1,...mono,fontSize:11,outline:"none"}}>{[["1","< 3 months"],["2","3–6 months"],["3","6–12 months"],["4","12–18 months"],["5","18+ months"]].map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:3,marginBottom:14}}>
          {PDIMS.map((dim,di)=>{const col=DCOLS[di];const val=fs[dim.id]||0;return(<div key={dim.id} style={{background:S2,border:"1px solid rgba(44,74,62,0.1)",padding:"10px 10px 12px"}}>
            ...
            <div style={{fontSize:10,color:T3,lineHeight:1.4,marginBottom:8}}>{dim.tip}</div>
      ...
      <div style={{...mono,fontSize:10,color:T1,letterSpacing:"0.1em",marginBottom:10}}>{initiatives.length} initiative{initiatives.length!==1?"s":""} added</div>
      {initiatives.length===0?(<div style={{padding:32,textAlign:"center",border:"1px dashed rgba(44,74,62,0.2)",background:S2}}><div style={{...serif,fontSize:14,color:T1,fontStyle:"italic",marginBottom:4}}>No initiatives added yet</div><div style={{fontSize:11,color:T1}}>Add your first above, or load sample data to explore the model</div></div>):<div style={{display:"flex",flexDirection:"column",gap:2,marginBottom:20}}>{initiatives.map(init=>{const tier=getTier(init.weightedScore);return(<div key={init.id} style={{background:S2,border:`1px solid ${BDR}`,padding:"12px 16px",display:"flex",alignItems:"center",gap:12}}><div style={{flex:1,...serif,fontSize:13,color:T2}}>{init.name}</div><div style={{...mono,fontSize:10,color:T1}}>{init.owner}</div><div style={{display:"flex",gap:4}}>{PDIMS.map((d,di)=><div key={d.id} style={{width:20,height:20,display:"flex",alignItems:"center",justifyContent:"center",...mono,fontSize:9,fontWeight:700,background:DCOLS[di]+"22",color:DCOLS[di],border:`1px solid ${DCOLS[di]}44`}}>{init.scores[d.id]}</div>)}</div><div style={{...mono,fontSize:11,color:tier.color,minWidth:40,textAlign:"right"}}>{init.weightedScore.toFixed(2)}</div><button onClick={()=>setInitiatives(p=>p.filter(i=>i.id!==init.id))} style={{background:"none",border:"none",color:T3,cursor:"pointer",fontSize:13}}>✕</button></div>);})}</div>}
    ...
    {view==="sequence"&&(<div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:2}}>{qs.map((q,qi)=><div key={qi} style={{border:`1px solid ${BDR}`,overflow:"hidden"}}><div style={{background:S3,padding:"10px 12px",borderBottom:"1px solid rgba(247,244,239,0.07)"}}><div style={{...mono,fontSize:10,color:T1,letterSpacing:"0.12em",textTransform:"uppercase"}}>{q.label}</div><div style={{fontSize:11,color:T3,marginTop:2}}>{q.sub} · {q.items.length} item{q.items.length!==1?"s":""}</div></div><div style={{padding:8,minHeight:80}}>{q.items.length===0?<div style={{...mono,fontSize:9,color:T2,textAlign:"center",paddingTop:16}}>—</div>:q.items.map(init=>{const tier=getTier(init.weightedScore);return(<div key={init.id} style={{borderLeft:`3px solid ${tier.color}`,background:S3,padding:"8px 10px",marginBottom:5}}><div style={{...serif,fontSize:11,color:T1,marginBottom:2}}>{init.name}</div><div style={{...mono,fontSize:8,color:tier.color}}>{init.owner} · {init.weightedScore.toFixed(2)}</div></div>);})}</div></div>)}</div>
    </div>)}
    {view==="resources"&&(<div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:2}}>{sorted.map(init=>{const tier=getTier(init.weightedScore);const dr=(6-init.scores.complexity)+(6-init.scores.risk)+(6-init.scores.capacity);const dp=Math.min(Math.round((dr/15)*100),100);const pp=Math.round((init.weightedScore/5)*100);const dc=dp>70?RED:dp>40?AMBER:G_MID;const dl=dp>70?"High Demand":dp>40?"Moderate":"Low Demand";return(<div key={init.id} style={{background:S2,border:`1px solid ${BDR}`,padding:"14px 18px",display:"grid",gridTemplateColumns:"200px 1fr auto",gap:14,alignItems:"center"}}><div><div style={{...serif,fontSize:13,color:T2}}>{init.name}</div><div style={{...mono,fontSize:9,color:T1,marginTop:2}}>{init.owner}</div></div><div>{[["Priority Score",pp,init.weightedScore.toFixed(1),tier.color],["Resource Demand",dp,dl.split(" ")[0],dc]].map(([label,pct,val,col])=><div key={label} style={{display:"flex",gap:8,alignItems:"center",marginBottom:5}}><span style={{...mono,fontSize:9,color:T3,width:110}}>{label}</span><div style={{flex:1,height:5,background:S4,borderRadius:3}}><div style={{height:"100%",width:`${pct}%`,background:col,borderRadius:3}}/></div><span style={{...mono,fontSize:10,color:col,width:36}}>{val}</span></div>)}</div><div style={{...mono,fontSize:10,color:dc,textAlign:"right"}}>{dl}</div></div>);})}
      </div>
    </div>)}
  </div>);
}
...
function Tool3DecisionStack(){
  ...
  function buildRadar(){
    ...
    [0.2,0.4,0.6,0.8,1].forEach(lv=>{const pts=DDIMS.map((_,i)=>{const a=Math.PI*2*i/n-Math.PI/2;return`${cx+Math.cos(a)*r*lv},${cy+Math.sin(a)*r*lv}`;}).join(" ");elems.push(<polygon key={`g${lv}`} points={pts} fill="none" stroke={G} strokeWidth="0.7" strokeOpacity="0.2"/>);});
    DDIMS.forEach((_,i)=>{const a=Math.PI*2*i/n-Math.PI/2;elems.push(<line key={`ax${i}`} x1={cx} y1={cy} x2={cx+Math.cos(a)*r} y2={cy+Math.sin(a)*r} stroke={G} strokeWidth="0.7" strokeOpacity="0.2"/>);});
  ...
  return(<div>
    ...
    {step==="setup"&&(<div>
      <div style={{background:S3,border:"1px solid rgba(212,168,71,0.15)",padding:24,marginBottom:24}}>
        <div style={{...serif,fontSize:18,fontWeight:700,color:T1,marginBottom:8}}>The Mental Model</div>
        <div style={{fontSize:13,color:T1,lineHeight:1.7,marginBottom:18}}>Most change decisions are held hostage by the wrong question — "should we do this?" The right question is "which version of this gives us the best return on disruption?" The Decision Stack forces three options into the same room and lets the evidence decide.</div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:2}}>{[["A","Full commitment","The complete change. Maximum investment, maximum potential. Name it honestly.",OCOLS[0],OLIGHTS[0]],["B","Partial or phased","Scoped-down or staged. Lower cost and risk — but does it still deliver enough value to justify the disruption?",OCOLS[1],OLIGHTS[1]],["C","Do nothing","Not absence of risk — presence of a different risk. Naming this forces the room to confront what inaction actually costs.",OCOLS[2],OLIGHTS[2]]].map(([letter,t,desc,col,bg])=><div key={letter} style={{background:bg,borderTop:`3px solid ${col}`,padding:"14px 16px"}}><div style={{...mono,fontSize:9,color:col,letterSpacing:"0.12em",textTransform:"uppercase",marginBottom:4}}>Option {letter}</div><div style={{...serif,fontSize:14,fontWeight:700,color:T1,marginBottom:6}}>{t}</div><div style={{fontSize:11,color:T2,lineHeight:1.6}}>{desc}</div></div>)}</div>
      </div>
      <Card style={{padding:22,marginBottom:12}}>
        <SL>Name the Decision</SL>
        <input value={title} onChange={e=>setTitle(e.target.value)} placeholder="e.g. Finance ERP Transformation" style={{width:"100%",background:S2,border:`1px solid ${BDR}`,padding:"9px 12px",color:T1,...sans,fontSize:13,outline:"none",marginBottom:18}}/>
        <SL>Name the Three Options</SL>
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>{opts.map((opt,i)=><div key={opt}><div style={{...mono,fontSize:9,color:OCOLS[i],letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:4}}>Option {opt.toUpperCase()} — {OLABELS[i]}</div><input value={names[i]} onChange={e=>{const n=[...names];n[i]=e.target.value;setNames(n);}} placeholder={["e.g. Full ERP implementation","e.g. Phase 1 only: GL & AP","e.g. Continue on legacy system"][i]} style={{width:"100%",background:OLIGHTS[i],border:`1px solid ${OCOLS[i]}33`,padding:"8px 11px",color:T1,...sans,fontSize:12,outline:"none",marginBottom:6}}/></div>)}</div>
      </Card>
    ...
    {step==="result"&&(<div>
      <div style={{borderLeft:`4px solid ${OCOLS[wi]}`,background:S3,padding:"24px 28px",marginBottom:2}}><div style={{...mono,fontSize:10,color:OCOLS[wi],letterSpacing:"0.14em",textTransform:"uppercase",marginBottom:6}}>The Stack recommends</div><div style={{...serif,fontSize:24,fontWeight:700,color:T1,marginBottom:8}}>{names[wi]||`Option ${winner.toUpperCase()}`}</div><div style={{fontSize:13,color:T2,lineHeight:1.7,maxWidth:580}}>{winner==="a"?"The full commitment option scores highest across the dimensions that matter. The investment appears proportionate to the strategic return.":winner==="b"?"The phased approach delivers meaningful value at lower risk. It may not capture the full strategic opportunity — but it is the option the organisation can most confidently execute.":"The analysis suggests that neither current change option clears the bar. The case for change needs strengthening before a resource commitment is made."}</div></div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:2,marginBottom:2}}>{opts.map((opt,i)=>{const col=OCOLS[i];const score=ws[opt];const isW=opt===winner;return(<div key={opt} style={{background:S2,border:`1px solid ${isW?"rgba(212,168,71,0.3)":"rgba(247,244,239,0.07)"}`,padding:22,position:"relative"}}>{isW&&<div style={{position:"absolute",top:-1,right:14,background:GOLD,color:T1,...mono,fontSize:8,letterSpacing:"0.1em",textTransform:"uppercase",padding:"2px 7px"}}>Recommended</div>}<div style={{...mono,fontSize:9,color:col,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:3}}>Option {opt.toUpperCase()}</div><div style={{...serif,fontSize:14,fontWeight:700,color:T1,marginBottom:14}}>{names[i]||OLABELS[i]}</div><div style={{...serif,fontSize:44,fontWeight:700,color:col,lineHeight:1}}>{score.toFixed(2)}</div><div style={{...mono,fontSize:9,color:T1,letterSpacing:"0.08em",marginBottom:16}}>/ 5.0</div>{DDIMS.map(d=>{const val=scores[opt][d.id]||0;return(<div key={d.id} style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}><span style={{fontSize:10,color:T1,flex:1}}>{d.label}</span><div style={{width:50,height:2,background:"rgba(247,244,239,0.08)"}}><div style={{height:"100%",width:`${(val/5)*100}%`,background:`${col}88`,borderBottom:`1.5px solid ${col}`}}/></div><span style={{...mono,fontSize:10,color:col,minWidth:12}}>{val}</span></div>);})}</div>);})}
      </div>
      <Card style={{padding:24,display:"flex",flexDirection:"column",alignItems:"center",marginBottom:2}}><SL>Dimension Profile — All Three Options</SL><svg width="300" height="300" viewBox="0 0 320 320">{buildRadar()}</svg><div style={{display:"flex",gap:18,marginTop:10}}>{opts.map((opt,i)=><div key={opt} style={{display:"flex",alignItems:"center",gap:6,...mono,fontSize:9,color:T2}}><div style={{width:8,height:8,borderRadius:"50%",background:OCOLS[i]}}/>{names[i]||`Option ${opt.toUpperCase()}`}</div>)}</div></Card>
      <Card style={{padding:24,marginBottom:2}}><SL>Before You Decide</SL><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>{[[OCOLS[0],"If you choose Option A",`${names[0]||"Option A"} — What does success look like in 12 months? Who owns the decision to stop if delivery fails?`],[OCOLS[1],"If you choose Option B",`${names[1]||"Option B"} — Is this genuinely strategic or a way to avoid the harder conversation? When is the decision point to scale up?`],[OCOLS[2],"If you choose Option C",`${names[2]||"Option C"} — What is the cost of inaction compounding over 12–24 months? Is this a decision to stop, or a decision to wait?`],["rgba(247,244,239,0.3)","Before any option","What assumption, if proven wrong, would change this decision? Who needs to own this decision — not just approve it? What would you need to see in 90 days to confirm you chose correctly?"]].map(([col,label,text])=><div key={label} style={{borderLeft:`3px solid ${col}`,background:S2,padding:"14px 16px"}}><div style={{...mono,fontSize:9,color:col,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:6}}>{label}</div><div style={{fontSize:12,color:T2,lineHeight:1.6}}>{text}</div></div>)}</div></Card>
      <div style={{background:"#F7F2E2",border:"1px solid rgba(212,168,71,0.3)",padding:24,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:16}}><div><div style={{...serif,fontSize:16,color:T1,marginBottom:4}}>Turn this into a decision your leadership team owns.</div><div style={{fontSize:12,color:T1}}>Strategy Made Real facilitates structured decision sessions that move from The Stack to a committed, documented decision in a single session.</div></div><div style={{display:"flex",gap:10,flexWrap:"wrap"}}><GBtn onClick={()=>exportDecisionStack({title,names,scores,ws,winner,dims:DDIMS})}>↓ Download Report</GBtn><OBtn onClick={()=>setStep("setup")}>← Start Over</OBtn></div></div>
    </div>)}
  </div>);
}
...
function Tool4RoleAnalyser(){
  ...
  async function analyse(){
    ...
    try{
      ...
      const response=await fetch("/api/analyse",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-5",max_tokens:4000,messages:[{role:"user",content:prompt}],system:"You are an expert organisational design consultant. Always respond with valid JSON only. No preamble, no markdown, no explanation. Start your response with { and end with }."})});
      const data=await response.json();
      const text=data.content?.[0]?.text||"";
      // Extract JSON - handle markdown wrapping and preamble text
      let jsonStr = text.replace(/```json\n?|```/g,"").trim();
      // Find first { and last } to extract just the JSON object
      const jsonStart = jsonStr.indexOf("{");
      const jsonEnd = jsonStr.lastIndexOf("}");
      if(jsonStart === -1 || jsonEnd === -1) throw new Error("No JSON found in response: " + jsonStr.slice(0,200));
      jsonStr = jsonStr.slice(jsonStart, jsonEnd+1);
      const parsed=JSON.parse(jsonStr);
      setResult(parsed);setOpen({"1":true});
    }catch(e){const errMsg = e?.message||"Unknown error";
      setError("Generation failed: " + errMsg);console.error("Full error:",e);}
    setLoading(false);
  }
  const sections=result?[
    {id:"1",num:"01",title:"Role Context & Purpose",content:(<div>
      <div style={{background:S4,border:"1px solid rgba(212,168,71,0.2)",padding:20,marginBottom:12}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          {[["Role Title",result.roleContext?.title||form.role],["Reports To",result.roleContext?.reportsTo||"—"],["Function",result.roleContext?.function||"—"],["Location",result.roleContext?.location||"—"]].map(([l,v])=><div key={l}><div style={{...mono,fontSize:9,color:GOLD,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:3}}>{l}</div><div style={{fontSize:13,color:T2}}>{v}</div></div>)}
        </div>
      </div>
      <div style={{marginBottom:10}}><div style={{...mono,fontSize:9,color:GOLD,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:6}}>Organisation Purpose</div><div style={{fontSize:13,color:T1,lineHeight:1.6}}>{result.purposeAndStrategy?.organisationPurpose}</div></div>
      <div><div style={{...mono,fontSize:9,color:GOLD,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:6}}>Strategic Contribution</div><div style={{fontSize:13,color:T1,lineHeight:1.6}}>{result.purposeAndStrategy?.roleContribution}</div></div>
    </div>)},
    {id:"2",num:"02",title:"Role Success Statement",content:(<div>
      <div style={{background:S2,border:"1px solid rgba(212,168,71,0.15)",padding:22,marginBottom:14}}><div style={{...serif,fontSize:14,color:T1,lineHeight:1.7}}>{result.roleSuccessStatement}</div></div>
      <div style={{background:S2,border:`1px solid ${BDR}`,padding:16}}><div style={{...mono,fontSize:9,color:GOLD,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:8}}>Success in This Business</div><div style={{fontSize:12,color:T1,lineHeight:1.7}}>{result.successInBusiness}</div></div>
    </div>)},
    {id:"3",num:"03",title:"Core Accountability Pillars",content:(<div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
      {result.accountabilityPillars?.map((p,i)=><div key={i} style={{background:S3,border:`1px solid ${GOLD}33`,borderTop:`3px solid ${GOLD}`,padding:"14px 16px"}}><div style={{...mono,fontSize:9,color:GOLD,marginBottom:4}}>{String(i+1).padStart(2,"0")}</div><div style={{...serif,fontSize:13,fontWeight:700,color:T2}}>{p}</div></div>)}
    </div>)},
    {id:"4",num:"04",title:"Responsibilities by Pillar",content:(<div style={{display:"flex",flexDirection:"column",gap:2}}>
      {result.responsibilitiesByPillar?.map((pillar,i)=><div key={i} style={{background:S2,border:`1px solid ${BDR}`,padding:"16px 20px",borderLeft:`3px solid ${GOLD}`}}>
        <div style={{...serif,fontSize:14,fontWeight:700,color:T1,marginBottom:4}}>{pillar.pillar}</div>
        <div style={{fontSize:12,color:T1,fontStyle:"italic",lineHeight:1.5,marginBottom:10}}>{pillar.purpose}</div>
        <ul style={{listStyle:"none"}}>{pillar.responsibilities?.map((r,j)=><li key={j} style={{fontSize:12,color:T1,lineHeight:1.5,paddingLeft:14,position:"relative",marginBottom:5}}><span style={{position:"absolute",left:0,color:GOLD}}>—</span>{r}</li>)}</ul>
      </div>)}
    </div>)},
    {id:"5",num:"05",title:"Key Competencies",content:(<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
      {result.keyCompetencies?.map((c,i)=><div key={i} style={{background:S2,border:`1px solid ${BDR}`,padding:"12px 14px",display:"flex",gap:10,alignItems:"flex-start"}}>
        <span style={{...mono,fontSize:11,color:GOLD,fontWeight:700,flexShrink:0}}>{String(i+1).padStart(2,"0")}</span>
        <div><div style={{...serif,fontSize:13,fontWeight:700,color:T1,marginBottom:3}}>{c.name}</div><div style={{fontSize:12,color:T1,lineHeight:1.5}}>{c.description}</div></div>
      </div>)}
    </div>)},
    {id:"6",num:"06",title:"What Success Looks Like",content:(<div>
      {result.whatSuccessLooksLike?.map((item,i)=><div key={i} style={{marginBottom:16,background:S2,border:`1px solid ${BDR}`,padding:"14px 18px"}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:8}}>
          <div><div style={{...mono,fontSize:9,color:GOLD,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:4}}>Area of Focus</div><div style={{...serif,fontSize:13,fontWeight:700,color:T2}}>{item.areaOfFocus}</div></div>
          <div><div style={{...mono,fontSize:9,color:GOLD,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:4}}>Purpose</div><div style={{fontSize:12,color:T1,lineHeight:1.5}}>{item.purpose}</div></div>
        </div>
        <div style={{...mono,fontSize:9,color:T1,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:6}}>Key Activities & Measures</div>
        <ul style={{listStyle:"none"}}>{item.keyActivities?.map((a,j)=><li key={j} style={{fontSize:12,color:T1,lineHeight:1.5,paddingLeft:14,position:"relative",marginBottom:4}}><span style={{position:"absolute",left:0,color:GOLD}}>—</span>{a}</li>)}</ul>
      </div>)}
    </div>)},
    {id:"7",num:"07",title:"Circle of Influence",content:(<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
      {[["Key Internal Relationships",result.circleOfInfluence?.internal,GOLD],["Key External Relationships",result.circleOfInfluence?.external,"#3a6b8a"]].map(([label,items,col])=><div key={label} style={{background:S2,border:`1px solid ${BDR}`,borderTop:`3px solid ${col}`,padding:16}}><div style={{...mono,fontSize:9,color:col,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:10}}>{label}</div>{items?.map((item,i)=><div key={i} style={{fontSize:12,color:T1,lineHeight:1.5,paddingLeft:12,position:"relative",marginBottom:4}}><span style={{position:"absolute",left:0,color:col,fontSize:10}}>—</span>{item}</div>)}</div>)}
    </div>)},
    {id:"8",num:"08",title:"Values & Behaviour Alignment",content:(<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
      {result.valuesAndBehaviours?.map((v,i)=><div key={i} style={{background:S2,border:`1px solid ${BDR}`,padding:"12px 14px"}}><div style={{...serif,fontSize:13,fontWeight:700,color:T1,marginBottom:4}}>{v.value}</div><div style={{fontSize:12,color:T2,lineHeight:1.5}}>{v.description}</div></div>)}
    </div>)},
    {id:"9",num:"09",title:"Time-Based Success Metrics",content:(<div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
      {[["Quarterly Priorities",result.timeBasedMetrics?.quarterly,GOLD],["6-Month Goals",result.timeBasedMetrics?.sixMonth,G_MID],["Annual Measures",result.timeBasedMetrics?.annual,"#3a6b8a"]].map(([label,items,col])=><div key={label} style={{background:S2,border:`1px solid ${BDR}`,borderTop:`3px solid ${col}`,padding:16}}><div style={{...mono,fontSize:9,color:col,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:10}}>{label}</div>{items?.map((item,i)=><div key={i} style={{display:"flex",gap:8,alignItems:"flex-start",marginBottom:6}}><span style={{...mono,fontSize:10,color:col,fontWeight:700,flexShrink:0}}>{i+1}</span><span style={{fontSize:12,color:T1,lineHeight:1.5}}>{item}</span></div>)}</div>)}
    </div>)},
  ]:[];
  return(<div style={{display:"grid",gridTemplateColumns:"300px 1fr",gap:0,minHeight:600}}>
    <div style={{borderRight:"1px solid rgba(44,74,62,0.12)",padding:"24px 20px",background:S2}}>
      <SL>Role Details</SL>
      {[["role","Role Title *","e.g. Head of Customer Experience"],["org","Organisation *","e.g. Regional Bank"],["level","Level / Seniority","e.g. Senior Manager"],["reports","Direct Reports","e.g. 4 direct, 12 total"]].map(([key,label,ph])=><div key={key} style={{marginBottom:10}}><div style={{...mono,fontSize:9,color:T1,letterSpacing:"0.09em",textTransform:"uppercase",marginBottom:4}}>{label}</div><input value={form[key]} onChange={e=>setForm(p=>({...p,[key]:e.target.value}))} placeholder={ph} style={{width:"100%",background:S2,border:`1px solid ${BDR}`,padding:"8px 11px",color:T1,...sans,fontSize:12,outline:"none"}}/></div>)}
      <div style={{height:1,background:"rgba(247,244,239,0.07)",margin:"16px 0"}}/>
      <SL>Context</SL>
      {[["context","Organisational Context","What is happening right now? Key priorities, changes underway, pressures..."],["pillars","Strategic Pillars","e.g. Customer first, operational excellence, digital transformation"],["challenges","Key Challenges","What are the biggest challenges this role needs to solve?"]].map(([key,label,ph])=><div key={key} style={{marginBottom:10}}><div style={{...mono,fontSize:9,color:T1,letterSpacing:"0.09em",textTransform:"uppercase",marginBottom:4}}>{label}</div><textarea value={form[key]} onChange={e=>setForm(p=>({...p,[key]:e.target.value}))} placeholder={ph} rows={3} style={{width:"100%",background:S2,border:`1px solid ${BDR}`,padding:"8px 11px",color:T1,...sans,fontSize:12,outline:"none",resize:"vertical",lineHeight:1.5}}/></div>)}
      ...
      {result&&<button onClick={()=>setResult(null)} style={{width:"100%",background:"transparent",border:`1px solid ${BDR}`,color:T3,padding:"9px",...mono,fontSize:10,letterSpacing:"0.1em",textTransform:"uppercase",cursor:"pointer",marginTop:8}}>Clear & Start Over</button>}
    </div>
    <div style={{padding:"24px 28px"}}>
      {loading&&<div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:400,textAlign:"center"}}><div style={{width:44,height:44,border:"3px solid rgba(212,168,71,0.15)",borderTop:`3px solid ${GOLD}`,borderRadius:"50%",animation:"spin 1s linear infinite",margin:"0 auto 18px"}}/><div style={{...mono,fontSize:11,color:T1,letterSpacing:"0.1em",textTransform:"uppercase"}}>Analysing role...</div><div style={{fontSize:12,color:T1,marginTop:6,fontStyle:"italic"}}>Building your strategy playbook</div></div>}
      {error&&<div style={{background:"#F0E0E0",border:"1px solid rgba(185,64,64,0.25)",padding:18,fontSize:13,color:T1,lineHeight:1.6}}>{error}</div>}
      {!loading&&!result&&!error&&<div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:400,textAlign:"center"}}><div style={{opacity:0.1,marginBottom:20}}><div style={{width:60,height:60,borderRadius:"50%",background:"#F5EED8",border:"2px solid rgba(212,168,71,0.2)"}}/></div><div style={{...serif,fontSize:20,color:T1,marginBottom:6}}>Enter role details to begin</div><div style={{fontSize:12,color:T3,lineHeight:1.6,maxWidth:280}}>Fill in the role title and organisation, add context, and generate a full role and strategy analysis.</div></div>}
      {result&&<div>
        <div style={{marginBottom:24,paddingBottom:18,borderBottom:"1px solid rgba(44,74,62,0.1)"}}><div style={{...serif,fontSize:26,fontWeight:700,color:T1,marginBottom:4}}>{form.role}</div><div style={{display:"flex",gap:10,flexWrap:"wrap",marginTop:8}}>{[form.org,form.level,form.reports].filter(Boolean).map((m,i)=><span key={i} style={{...mono,fontSize:10,color:T2,background:S3,border:`1px solid ${BDR}`,padding:"2px 9px"}}>{m}</span>)}</div></div>
        {sections.map(sec=><div key={sec.id} style={{marginBottom:2,background:S2,border:"1px solid rgba(44,74,62,0.1)"}}><div onClick={()=>setOpen(p=>({...p,[sec.id]:!p[sec.id]}))} style={{padding:"14px 18px",cursor:"pointer",display:"flex",alignItems:"center",gap:10}}><span style={{...mono,fontSize:10,color:GOLD}}>{sec.num}</span><span style={{...serif,fontSize:15,fontWeight:700,color:T2}}>{sec.title}</span><span style={{marginLeft:"auto",color:T3,fontSize:10,transform:open[sec.id]?"rotate(180deg)":"none",transition:"transform 0.2s"}}>▼</span></div>{open[sec.id]&&<div style={{padding:"0 18px 18px"}}>{sec.content}</div>}</div>)}
      ...
      {view === "builder" && (
        <div>
          ...
          <div style={{marginBottom:16}}>
            <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:T1,letterSpacing:"0.09em",textTransform:"uppercase",marginBottom:4}}>Organisation / Programme Name</div>
            <input value={orgName} onChange={e=>setOrgName(e.target.value)} placeholder="e.g. Customer Transformation Programme" style={{background:S2,border:`1px solid ${BDR}`,padding:"8px 12px",color:T1,fontFamily:"'DM Sans',sans-serif",fontSize:13,outline:"none",width:400}} />
          </div>
              ...
              return (
                <div key={m.id} onClick={() => toggleMeeting(m.id)} style={{
                  ...
                  <div style={{width:22,height:22,borderRadius:"50%",border:`2px solid ${isOn?col:"rgba(247,244,239,0.2)"}`,background:isOn?col:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                    {isOn && <span style={{color:T1,fontSize:10,fontWeight:700}}>✓</span>}
                  </div>
                  <div>
                    <div style={{fontFamily:"'Playfair Display',serif",fontSize:14,fontWeight:700,color:T1,marginBottom:2}}>{m.label}</div>
                    <div style={{fontSize:11,color:T2,lineHeight:1.5}}>{m.purpose}</div>
                  </div>
                  ...
                  <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:T1,whiteSpace:"nowrap"}}>{m.duration}</div>
                  <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:T1,whiteSpace:"nowrap"}}>{m.owner}</div>
                </div>
              );
            })}
          </div>
          ...
          {customMeetings.map(m => (
            <div key={m.id} style={{background:S2,border:"1px solid rgba(212,168,71,0.15)",padding:"12px 18px",display:"flex",alignItems:"center",gap:12,marginBottom:2}}>
              <div onClick={() => toggleMeeting(m.id)} style={{width:22,height:22,borderRadius:"50%",border:`2px solid ${selected.includes(m.id)?GOLD:"rgba(247,244,239,0.2)"}`,background:selected.includes(m.id)?GOLD:"transparent",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0}}>
                {selected.includes(m.id) && <span style={{color:T1,fontSize:10,fontWeight:700}}>✓</span>}
              </div>
              <div style={{flex:1}}>
                <div style={{fontFamily:"'Playfair Display',serif",fontSize:13,color:T2}}>{m.label}</div>
                <div style={{fontSize:11,color:T1}}>{m.cadence} · {m.duration} · {m.owner}</div>
              </div>
              <button onClick={() => setCustomMeetings(p => p.filter(x => x.id!==m.id))} style={{background:"none",border:"none",color:T3,cursor:"pointer",fontSize:13}}>✕</button>
            </div>
          ))}

          {addingCustom ? (
            <div style={{background:S3,border:"1px solid rgba(212,168,71,0.2)",padding:20,marginTop:8}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
                {[["label","Meeting Name","e.g. Change Champion Sync"],["owner","Owner","e.g. Change Lead"],["attendees","Attendees","e.g. Change champions, team leads"],["duration","Duration","e.g. 45 min"]].map(([key,label,ph]) => (
                  <div key={key}>
                    <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:T1,letterSpacing:"0.09em",textTransform:"uppercase",marginBottom:4}}>{label}</div>
                    <input value={customForm[key]} onChange={e => setCustomForm(p=>({...p,[key]:e.target.value}))} placeholder={ph} style={{width:"100%",background:S2,border:`1px solid ${BDR}`,padding:"7px 10px",color:T1,fontFamily:"'DM Sans',sans-serif",fontSize:12,outline:"none"}} />
                  </div>
                ))}
                <div style={{gridColumn:"1/-1"}}>
                  <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:T1,letterSpacing:"0.09em",textTransform:"uppercase",marginBottom:4}}>Purpose</div>
                  <input value={customForm.purpose} onChange={e => setCustomForm(p=>({...p,purpose:e.target.value}))} placeholder="What does this meeting exist to do?" style={{width:"100%",background:S2,border:`1px solid ${BDR}`,padding:"7px 10px",color:T1,fontFamily:"'DM Sans',sans-serif",fontSize:12,outline:"none"}} />
                </div>
                <div>
                  <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:T1,letterSpacing:"0.09em",textTransform:"uppercase",marginBottom:4}}>Cadence</div>
                  <select value={customForm.cadence} onChange={e=>setCustomForm(p=>({...p,cadence:e.target.value}))} style={{width:"100%",background:S1,border:`1px solid ${BDR}`,padding:"7px 10px",color:T1,fontFamily:"'DM Mono',monospace",fontSize:11,outline:"none"}}>
            ...
            <button onClick={() => setAddingCustom(true)} style={{background:"transparent",border:"1px dashed rgba(44,74,62,0.25)",color:T1,padding:"10px 18px",fontFamily:"'DM Mono',monospace",fontSize:10,letterSpacing:"0.1em",textTransform:"uppercase",cursor:"pointer",marginTop:8}}>+ Add Custom Meeting</button>
          )}
      ...
      {view === "calendar" && (
        <div>
          <div style={{marginBottom:20}}>
            <div style={{fontFamily:"'Playfair Display',serif",fontSize:20,fontWeight:700,color:T1,marginBottom:4}}>{orgName || "Programme"} — Operating Rhythm</div>
            <div style={{fontSize:12,color:T1}}>Weekly recurring meetings shown below. Monthly and quarterly cadences listed separately.</div>
          </div>

          {/* Weekly grid */}
          <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:2,marginBottom:24}}>
            {WEEK_DAYS.map(day => (
              <div key={day} style={{border:`1px solid ${BDR}`,overflow:"hidden"}}>
                <div style={{background:S4,padding:"10px 12px",borderBottom:"1px solid rgba(247,244,239,0.07)"}}>
                  <div style={{fontFamily:"'DM Mono',monospace",fontSize:11,color:T1,letterSpacing:"0.12em",textTransform:"uppercase"}}>{day}</div>
                </div>
                <div style={{padding:8,minHeight:80}}>
                  {byDay[day].filter((m,i,arr) => arr.findIndex(x=>x.id===m.id)===i).map(m => {
                    ...
                    const col = RHYTHM_COLOURS[cadence];
                    return (
                      <div key={m.id} style={{borderLeft:`3px solid ${col}`,background:S3,padding:"8px 10px",marginBottom:5}}>
                        <div style={{fontFamily:"'Playfair Display',serif",fontSize:11,color:T1,marginBottom:2}}>{m.label}</div>
                        <div style={{fontFamily:"'DM Mono',monospace",fontSize:8,color:col}}>{m.duration}</div>
                      </div>
                    );
                  })}
                  {byDay[day].length === 0 && <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:T1,textAlign:"center",paddingTop:16}}>—</div>}
                </div>
              </div>
            ))}
          </div>
            ...
            <Card style={{padding:20,marginBottom:16}}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                {monthlyCadence.map(m => {
                  ...
                  return (
                    <div key={m.id} style={{display:"grid",gridTemplateColumns:"120px 1fr auto",gap:14,alignItems:"center",padding:"10px 14px",background:S2,border:`1px solid rgba(247,244,239,0.07)`}}>
                      ...
                      <div>
                        <div style={{fontFamily:"'Playfair Display',serif",fontSize:13,color:T1,marginBottom:2}}>{m.label}</div>
                        <div style={{fontSize:11,color:T1}}>{m.purpose}</div>
                      </div>
                      <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:T1,textAlign:"right"}}>
                        <div>{m.duration}</div>
      ...
      {view === "register" && (
        <div>
          <Card style={{padding:0,overflow:"hidden"}}>
            <DataTable
              ...
              rows={allMeetings.map(m => [
                <span style={{fontFamily:"'Playfair Display',serif",fontSize:13,color:T1,fontWeight:700}}>{m.label}</span>,
          ...
          {allMeetings.length === 0 && (
            <div style={{textAlign:"center",padding:40,color:T3,fontStyle:"italic",fontSize:13}}>No meetings selected. Go to the Builder tab to add meetings.</div>
          )}
        </div>
      )}
        ...
        <div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:2}}>
            {[
              ...
              <div key={title} style={{background:S2,border:`1px solid ${BDR}`,borderTop:`3px solid ${col}`,padding:20}}>
                <div style={{fontFamily:"'Playfair Display',serif",fontSize:14,fontWeight:700,color:T1,marginBottom:8}}>{title}</div>
                <div style={{fontSize:12,color:T2,lineHeight:1.7}}>{text}</div>
              </div>
            ))}
          </div>
        </div>
    ...
    <div>
      ...
      {view === "builder" && (
        <div>
          ...
          <div style={{display:"flex",gap:2,marginBottom:16,flexWrap:"wrap"}}>
            {KPI_PILLARS.map(p => (
              <button key={p.id} onClick={() => setActivePillar(p.id)} style={{
                background: activePillar===p.id ? `${p.color}22` : S2,
              ...
              return (
                <div key={idx} onClick={() => toggleKPI(activePillar, idx)} style={{
                  ...
                  <div style={{width:20,height:20,borderRadius:"50%",border:`2px solid ${isOn?col:"rgba(247,244,239,0.2)"}`,background:isOn?col:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                    {isOn && <span style={{color:T1,fontSize:9,fontWeight:700}}>✓</span>}
                  </div>
                  <div>
                    <div style={{fontFamily:"'Playfair Display',serif",fontSize:13,fontWeight:700,color:T1,marginBottom:3}}>{kpi.name}</div>
                    <div style={{fontSize:11,color:T1,fontFamily:"'DM Mono',monospace"}}>Target: {kpi.target}</div>
                  </div>
                  <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:T1,whiteSpace:"nowrap"}}>{kpi.frequency}</div>
          ...
          {customKPIs.length > 0 && (
            <div style={{marginBottom:12}}>
              ...
              {customKPIs.map(k => {
                ...
                return (
                  <div key={k.id} style={{background:S2,border:"1px solid rgba(212,168,71,0.15)",padding:"12px 16px",display:"grid",gridTemplateColumns:"26px 1fr auto",gap:12,alignItems:"center",marginBottom:2}}>
                    ...
                    <div>
                      <div style={{fontFamily:"'Playfair Display',serif",fontSize:13,color:T2}}>{k.name}</div>
                      <div style={{fontSize:11,color:T1}}>{k.pillar} · {k.frequency} · Target: {k.target}</div>
                    </div>
                    <button onClick={() => setCustomKPIs(p=>p.filter(x=>x.id!==k.id))} style={{background:"none",border:"none",color:T3,cursor:"pointer",fontSize:13}}>✕</button>
                  </div>
                );
              })}
            </div>
          ...
          {addingCustom ? (
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
                {[["name","KPI Name","e.g. Supplier on-time delivery rate"],["formula","Formula / Calculation","e.g. On-time deliveries ÷ Total deliveries × 100"],["target","Target","e.g. ≥ 95%"],["frequency","Measurement Frequency","e.g. Weekly"]].map(([key,label,ph]) => (
                  <div key={key} style={{gridColumn:key==="name"||key==="formula"?"1/-1":"auto"}}>
                    <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:T1,letterSpacing:"0.09em",textTransform:"uppercase",marginBottom:4}}>{label}</div>
                    <input value={cForm[key]} onChange={e=>setCForm(p=>({...p,[key]:e.target.value}))} placeholder={ph} style={{width:"100%",background:S2,border:`1px solid ${BDR}`,padding:"7px 10px",color:T1,fontFamily:"'DM Sans',sans-serif",fontSize:12,outline:"none"}} />
                  </div>
                ))}
                <div>
                  <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:T1,letterSpacing:"0.09em",textTransform:"uppercase",marginBottom:4}}>Pillar</div>
                  <select value={cForm.pillar} onChange={e=>setCForm(p=>({...p,pillar:e.target.value}))} style={{width:"100%",background:S1,border:`1px solid ${BDR}`,padding:"7px 10px",color:T1,fontFamily:"'DM Mono',monospace",fontSize:11,outline:"none"}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:10,paddingTop:20}}>
                  ...
                  <label htmlFor="lead-toggle" style={{fontSize:12,color:T1,cursor:"pointer"}}>Lead indicator (predictive)</label>
                </div>
              </div>
              <div style={{display:"flex",gap:8}}>
            ...
            <button onClick={() => setAddingCustom(true)} style={{background:"transparent",border:"1px dashed rgba(44,74,62,0.25)",color:T1,padding:"10px 18px",fontFamily:"'DM Mono',monospace",fontSize:10,letterSpacing:"0.1em",textTransform:"uppercase",cursor:"pointer",marginTop:8}}>+ Add Custom KPI</button>
          )}
      ...
      {view === "scorecard" && (
        <div>
            ...
            return (
              <div key={pillar.id} style={{marginBottom:20}}>
                <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
                  ...
                  <div style={{fontFamily:"'Playfair Display',serif",fontSize:16,fontWeight:700,color:T2}}>{pillar.label}</div>
                ...
                <Card style={{padding:0,overflow:"hidden"}}>
                  <DataTable
                    ...
                    rows={pillKPIs.map(k => [
                      <span style={{fontFamily:"'Playfair Display',serif",fontSize:13,fontWeight:700,color:T2}}>{k.name}</span>,
                      ...
                      <span style={{fontFamily:"'DM Mono',monospace",fontSize:10,color:T1}}>{k.formula}</span>,
                    ])}
                  />
                </Card>
              </div>
          ...
          {allSelected.length === 0 && <div style={{textAlign:"center",padding:40,color:T3,fontStyle:"italic",fontSize:13}}>No KPIs selected. Go to the Builder tab to add KPIs.</div>}
        </div>
      )}
        ...
        <div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            <div>
              ...
              {leadKPIs.length === 0 ? <div style={{fontSize:12,color:T1,fontStyle:"italic",padding:"20px 0"}}>No lead indicators selected yet. Add KPIs marked as lead indicators.</div> :
                leadKPIs.map((k,i) => (
                  <div key={i} style={{background:"#E8F0EB",border:"1px solid rgba(74,122,104,0.15)",borderLeft:`3px solid ${G_MID}`,padding:"12px 14px",marginBottom:6}}>
                    <div style={{fontFamily:"'Playfair Display',serif",fontSize:13,color:T1,marginBottom:3}}>{k.name}</div>
                    <div style={{fontSize:11,color:T2}}>{k.frequency} · Target: {k.target}</div>
                  </div>
                ))
              }
            </div>
            <div>
              ...
              {lagKPIs.length === 0 ? <div style={{fontSize:12,color:T1,fontStyle:"italic",padding:"20px 0"}}>No lag indicators selected yet.</div> :
                lagKPIs.map((k,i) => (
                  <div key={i} style={{background:"#F7F2E2",border:"1px solid rgba(200,154,42,0.15)",borderLeft:`3px solid ${AMBER}`,padding:"12px 14px",marginBottom:6}}>
                    <div style={{fontFamily:"'Playfair Display',serif",fontSize:13,color:T1,marginBottom:3}}>{k.name}</div>
                    <div style={{fontSize:11,color:T2}}>{k.frequency} · Target: {k.target}</div>
                  </div>
                ))
            ...
            {CAP_DOMAINS.map(d => {
              ...
              return (
                <div key={d.id} onClick={() => setActiveDomain(d.id)} style={{
                  background: isActive ? S3 : S2,
                  border: `1px solid ${isActive ? (col+"55") : "rgba(247,244,239,0.07)"}`,
                  padding:"12px 14px",
                  cursor:"pointer",
                  transition:"all 0.15s",
                }}>
                  <div style={{fontFamily:"'Playfair Display',serif",fontSize:13,color:T1,marginBottom:4}}>{d.label}</div>
          ...
          {(() => {
            ...
            return (
              <Card style={{padding:24}}>
                <div style={{fontFamily:"'Playfair Display',serif",fontSize:18,fontWeight:700,color:T1,marginBottom:6}}>{dom.label}</div>
                <div style={{fontSize:12,color:T1,lineHeight:1.6,marginBottom:20,fontStyle:"italic"}}>{dom.desc}</div>
                    ...
                    {CAP_LEVELS.map(lvl => {
                      ...
                      return (
                        <div key={lvl.score} onClick={() => setScores(p => ({...p,[dom.id]:lvl.score}))} style={{
                          ...
                          {isOn && <div style={{position:"absolute",top:7,right:7,width:12,height:12,borderRadius:"50%",background:lvl.color,display:"flex",alignItems:"center",justifyContent:"center",fontSize:7,color:T1,fontWeight:700}}>✓</div>}
                          ...
                          <div style={{fontSize:10,color:T2,lineHeight:1.5}}>{lvl.desc}</div>
                        </div>
                      );
                    })}
                  </div>
                ...
                <div>
                  <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:T1,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:6}}>Notes (optional)</div>
                  <textarea value={notes[dom.id]||""} onChange={e=>setNotes(p=>({...p,[dom.id]:e.target.value}))} placeholder="What is driving this score? What specific evidence or examples?" rows={2} style={{width:"100%",background:S2,border:`1px solid ${BDR}`,padding:"8px 11px",color:T1,fontFamily:"'DM Sans',sans-serif",fontSize:12,outline:"none",resize:"vertical",lineHeight:1.5}} />
                </div>
              </Card>
            );
          })()}
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:2,marginBottom:24}}>
            {[
              ...
              <Card key={i} style={{padding:20}}>
                ...
                <div style={{fontSize:11,color:T1,marginTop:4}}>{s.sub}</div>
              </Card>
            ))}
          </div>
              ...
              {CAP_DOMAINS.map(dom => {
                ...
                return (
                  <div key={dom.id} style={{display:"grid",gridTemplateColumns:"180px 1fr 60px 100px",gap:12,alignItems:"center",padding:"8px 0",borderBottom:"1px solid rgba(247,244,239,0.05)"}}>
                    <div style={{fontFamily:"'Playfair Display',serif",fontSize:12,color:T2}}>{dom.label}</div>
                    <div>
                      <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                        <span style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:T3}}>Capability</span>
                        <span style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:col}}>{s>0?s:"-"}/5</span>
                      </div>
                      <div style={{height:6,background:S4,borderRadius:2}}>
                        <div style={{height:"100%",width:`${barW}%`,background:col,borderRadius:2,transition:"width 0.6s"}} />
                      </div>
                    </div>
                    <div style={{textAlign:"center"}}>
                      <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:T3,marginBottom:2}}>Imp.</div>
                    ...
                    <div style={{textAlign:"right"}}>
                      ...
                      ) : <span style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:T3}}>Not assessed</span>}
                    </div>
                  </div>
                );
              })}
      ...
      {view === "priorities" && (
        <div>
          {gaps.length === 0 && critGaps.length === 0 ? (
            <div style={{textAlign:"center",padding:48,color:T3,fontStyle:"italic",fontSize:13}}>No high-priority gaps identified. Assess all domains first, or your organisation is in strong shape.</div>
          ) : (
            <>
              {critGaps.length > 0 && (
                <div style={{background:"#F5E8E8",border:"1px solid rgba(185,64,64,0.25)",padding:24,marginBottom:12}}>
                  ...
                  {critGaps.map(dom => (
                    <div key={dom.id} style={{borderLeft:`2px solid ${RED}`,paddingLeft:16,marginBottom:16}}>
                      <div style={{fontFamily:"'Playfair Display',serif",fontSize:14,color:T1,marginBottom:4}}>{dom.label}</div>
                      <div style={{fontSize:12,color:T1,lineHeight:1.6,marginBottom:4}}>{dom.desc}</div>
                ...
                <Card style={{padding:24,marginBottom:12}}>
                  ...
                  {gaps.filter(d=>scores[d.id]===2).map(dom => (
                    <div key={dom.id} style={{borderLeft:`2px solid #C0622A`,paddingLeft:16,marginBottom:14}}>
                      <div style={{fontFamily:"'Playfair Display',serif",fontSize:14,color:T1,marginBottom:3}}>{dom.label}</div>
                      <div style={{fontSize:12,color:T1,lineHeight:1.6}}>{dom.desc}</div>
                    </div>
                  ))}
                </Card>
              )}
              {strengths.length > 0 && (
                <Card style={{padding:24}}>
                  ...
                  <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
                    {strengths.map(dom => (
                      <div key={dom.id} style={{fontFamily:"'DM Mono',monospace",fontSize:10,color:G_MID,background:"#DDE9E4",border:"1px solid rgba(74,122,104,0.2)",padding:"4px 12px"}}>
      ...
      {view === "actions" && (
        <div>
          ...
          {CAP_DOMAINS.filter(d => scores[d.id] > 0 && scores[d.id] < 4).map(dom => {
            ...
            return (
              <div key={dom.id} style={{background:S2,border:`1px solid ${BDR}`,borderLeft:`3px solid ${col}`,padding:"16px 20px",marginBottom:2}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8,flexWrap:"wrap",gap:8}}>
                  <div style={{fontFamily:"'Playfair Display',serif",fontSize:14,fontWeight:700,color:T2}}>{dom.label}</div>
                ...
                <div style={{fontSize:12,color:T2,lineHeight:1.6}}>{dom.desc}</div>
              </div>
            );
          })}
          {CAP_DOMAINS.filter(d => scores[d.id] > 0 && scores[d.id] < 4).length === 0 && (
            <div style={{textAlign:"center",padding:48,color:T3,fontStyle:"italic",fontSize:13}}>No gaps to action. All assessed domains are at Capable or Strong.</div>
          )}
        </div>
      )}
    </div>
      ...
      {view === "setup" && (
        <div>
          ...
          <Card style={{padding:24,marginBottom:12}}>
            ...
            <div style={{marginBottom:12}}>
              <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:T1,letterSpacing:"0.09em",textTransform:"uppercase",marginBottom:4}}>Change / Initiative Name</div>
              <input value={changeName} onChange={e=>setChangeName(e.target.value)} placeholder="e.g. Customer Service Process Redesign" style={{width:"100%",background:S2,border:`1px solid ${BDR}`,padding:"9px 12px",color:T1,fontFamily:"'DM Sans',sans-serif",fontSize:13,outline:"none"}} />
            </div>
            <div style={{marginBottom:12}}>
              <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:T1,letterSpacing:"0.09em",textTransform:"uppercase",marginBottom:8}}>Type of Change</div>
              <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                {CHANGE_TYPES.map(t => (
                  <div key={t.id} onClick={() => setChangeType(t.id)} style={{padding:"8px 14px",cursor:"pointer",border:`1px solid ${changeType===t.id?G_MID+"55":"rgba(247,244,239,0.1)"}`,background:changeType===t.id?"rgba(74,122,104,0.12)":S2,color:changeType===t.id?S1:"rgba(247,244,239,0.4)",transition:"all 0.15s"}}>
            ...
            <div style={{marginBottom:12}}>
              <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:T1,letterSpacing:"0.09em",textTransform:"uppercase",marginBottom:4}}>Impacted Groups</div>
              <textarea value={impactedGroups} onChange={e=>setImpactedGroups(e.target.value)} placeholder="e.g. Customer service team (45 people), Team leaders (8), Operations manager" rows={2} style={{width:"100%",background:S2,border:`1px solid ${BDR}`,padding:"8px 11px",color:T1,fontFamily:"'DM Sans',sans-serif",fontSize:12,outline:"none",resize:"vertical",lineHeight:1.5}} />
            </div>
            <div>
              <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:T1,letterSpacing:"0.09em",textTransform:"uppercase",marginBottom:8}}>Scale of Change (1 = minor, 5 = transformational)</div>
              <div style={{display:"flex",alignItems:"center",gap:14}}>
        ...
        <div>
          ...
          <div style={{display:"flex",gap:2,marginBottom:16}}>
            {ADKAR_ELEMENTS.map(el => {
              ...
              const col = el.color;
              return (
                <div key={el.id} onClick={() => setActiveAdkar(el.id)} style={{flex:1,padding:"10px 8px",cursor:"pointer",border:`1px solid ${isActive?col+"55":"rgba(247,244,239,0.07)"}`,background:isActive?S3:S2,textAlign:"center",transition:"all 0.15s"}}>
          ...
          {(() => {
            ...
            return (
              <Card style={{padding:24}}>
                <div style={{fontFamily:"'Playfair Display',serif",fontSize:18,fontWeight:700,color:T1,marginBottom:4}}>{el.label}</div>
                <div style={{fontSize:12,color:T1,lineHeight:1.6,marginBottom:14}}>{el.desc}</div>
                <div style={{background:S3,borderLeft:`3px solid ${el.color}`,padding:"10px 14px",marginBottom:18,fontSize:11,color:T2,lineHeight:1.6,fontStyle:"italic"}}>{el.barrier}</div>
                  ...
                  {[1,2,3,4,5].map(n => {
                    ...
                    return (
                      <div key={n} onClick={() => setAdkarScores(p=>({...p,[el.id]:n}))} style={{padding:"12px 8px",cursor:"pointer",border:`2px solid ${isOn?el.color:"transparent"}`,background:`${el.color}${isOn?"22":"0a"}`,textAlign:"center",transition:"all 0.12s"}}>
                        ...
                        <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:isOn?el.color:T3,textTransform:"uppercase",letterSpacing:"0.07em",marginTop:4}}>{labels[n]}</div>
                      </div>
                    );
                  })}
                </div>
      ...
      {view === "resistance" && (
        <div>
              ...
              return (
                <div key={r.id} onClick={() => toggleResistance(r.id)} style={{
                  ...
                  <div style={{width:18,height:18,borderRadius:"50%",border:`2px solid ${isOn?riskCol:"rgba(247,244,239,0.2)"}`,background:isOn?riskCol:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                    {isOn && <span style={{color:T1,fontSize:8,fontWeight:700}}>✓</span>}
                  </div>
                  <div style={{fontSize:13,color:isOn?T1:T4}}>{r.label}</div>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <span style={{fontFamily:"'DM Mono',monospace",fontSize:10,color:T1}}>{resistanceFactors.length} factor{resistanceFactors.length!==1?"s":""} identified</span>
        ...
        <div>
          ...
          {adkarComplete && (
            <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:2,marginBottom:20}}>
              {ADKAR_ELEMENTS.map(el => {
                ...
                return (
                  <div key={el.id} style={{
                    background: isBarrier ? `${el.color}18` : S2,
                    border: `1px solid ${isBarrier ? el.color+"55" : "rgba(247,244,239,0.07)"}`,
                    padding:"14px 12px",
                    textAlign:"center",
                    position:"relative",
                  }}>
                    {isBarrier && <div style={{position:"absolute",top:-1,left:"50%",transform:"translateX(-50%)",background:el.color,color:T1,fontFamily:"'DM Mono',monospace",fontSize:7,letterSpacing:"0.08em",textTransform:"uppercase",padding:"1px 8px",whiteSpace:"nowrap"}}>Barrier Point</div>}
                    ...
                    <div style={{fontFamily:"'DM Mono',monospace",fontSize:8,color:T3,marginTop:2}}>/5</div>
                  </div>
                );
              })}
            </div>
          ...
          <Card style={{padding:24,marginBottom:12}}>
              ...
              return (
                <div key={el.id} style={{marginBottom:16,paddingBottom:16,borderBottom:"1px solid rgba(247,244,239,0.05)"}}>
                  <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
                    ...
                    <span style={{fontFamily:"'Playfair Display',serif",fontSize:14,fontWeight:700,color:T2}}>{el.label}</span>
                    ...
                    {el.id === weakestADKAR?.id && <span style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:RED,background:"#EDD8D8",border:"1px solid rgba(185,64,64,0.3)",padding:"2px 7px"}}>Barrier point</span>}
                  </div>
                  {interventions[el.id].slice(0, s===1?5:s===2?4:3).map((action,i) => (
                    <div key={i} style={{display:"flex",gap:8,alignItems:"flex-start",marginBottom:5}}>
                      ...
                      <span style={{fontSize:12,color:T2,lineHeight:1.6}}>{action}</span>
                    </div>
                  ))}
                </div>
              );
            ...
            {ADKAR_ELEMENTS.every(el => (adkarScores[el.id]||0) >= 4) && (
              <div style={{textAlign:"center",padding:24,color:T1,fontStyle:"italic",fontSize:13}}>Strong ADKAR scores across all elements. Focus on reinforcement to sustain adoption.</div>
            )}
          </Card>
                ...
                return (
                  <div key={id} style={{borderLeft:`2px solid ${riskCol}`,paddingLeft:16,marginBottom:14}}>
                    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:5}}>
                      <div style={{fontFamily:"'Playfair Display',serif",fontSize:13,fontWeight:700,color:T2}}>{factor.label}</div>
                    ...
                    <div style={{fontSize:12,color:T2,lineHeight:1.6}}>{responses[id]}</div>
                  </div>
                );
              })}
            </Card>
...
// ═══ MAIN APP SHELL ════════════════════════════════════════════════════════════
const TOOLS=[
  {id:"diagnostic",     num:"01",label:"Strategy Diagnostic",       sub:"Are we clear?"},
  {id:"prioritisation", num:"02",label:"Initiative Prioritisation",  sub:"What matters most?"},
  {id:"decision",       num:"03",label:"The Decision Stack",         sub:"What should we choose?"},
  {id:"role",           num:"04",label:"Role & Strategy Analyser",   sub:"Who owns what?"},
  {id:"rhythm",         num:"05",label:"Operating Rhythm Builder",   sub:"How do we run it?"},
  {id:"kpi",            num:"06",label:"KPI & Performance Builder",  sub:"How do we measure it?"},
  {id:"capability",     num:"07",label:"Capability & Gap Mapper",    sub:"Can we actually do it?"},
  {id:"change",         num:"08",label:"Change & Adoption Planner",  sub:"Will people adopt it?"},
];
...
export default function SMRSuite(){
  ...
  return(<div style={{minHeight:"100vh",background:S1,display:"flex",flexDirection:"column"}}>
    ...
    {/* ── Header ── */}
    <header style={{background:G,borderBottom:"none",padding:"14px 28px",display:"flex",justifyContent:"space-between",alignItems:"center",position:"sticky",top:0,zIndex:200,flexShrink:0}}>
      <div style={{display:"flex",alignItems:"center",gap:14}}>
        <svg width="32" height="32" viewBox="0 0 120 120" fill="none">
          <ellipse cx="62" cy="60" rx="48" ry="52" stroke={S1} strokeWidth="1.5" strokeOpacity="0.6"/>
          <ellipse cx="68" cy="60" rx="32" ry="52" stroke={S1} strokeWidth="1.5" strokeOpacity="0.6"/>
          <ellipse cx="62" cy="60" rx="18" ry="20" stroke={S1} strokeWidth="1.5" strokeOpacity="0.6"/>
          <circle cx="74" cy="60" r="10" fill={GOLD}/>
          <circle cx="74" cy="60" r="5" fill={G} fillOpacity="0.2"/>
          <line x1="38" y1="28" x2="44" y2="90" stroke={S1} strokeWidth="1.2" strokeOpacity="0.5"/>
        </svg>
        <div>
          <div style={{...serif,fontSize:14,fontWeight:700,color:T2,lineHeight:1.1}}>STRATEGY <em style={{fontWeight:400,color:GOLD}}>Made Real</em></div>
          <div style={{...mono,fontSize:8,color:"rgba(247,244,239,0.5)",letterSpacing:"0.12em",textTransform:"uppercase",marginTop:2}}>Beyond theory. Into delivery. Powered by people.</div>
        </div>
      </div>
      <div style={{...mono,fontSize:9,color:"rgba(247,244,239,0.4)",letterSpacing:"0.1em",textTransform:"uppercase",textAlign:"right",lineHeight:1.8}}>Strategy Execution Suite</div>
    </header>
    ...
    <div style={{display:"flex",flex:1,overflow:"hidden"}}>
      ...
      {/* ── Sidebar ── */}
      <nav style={{width:236,flexShrink:0,background:G,borderRight:"none",overflowY:"auto",position:"sticky",top:57,height:"calc(100vh - 57px)",display:"flex",flexDirection:"column"}}>
        <div style={{padding:"10px 0",flex:1}}>
          {TOOLS.map(t=>(
            <button key={t.id} onClick={()=>setActiveTool(t.id)} style={{width:"100%",background:activeTool===t.id?"rgba(212,168,71,0.15)":"transparent",border:"none",borderLeft:activeTool===t.id?`3px solid ${GOLD}`:`3px solid transparent`,padding:"11px 16px",cursor:"pointer",textAlign:"left",transition:"background 0.15s",display:"flex",gap:10,alignItems:"center"}}>
              <div style={{minWidth:0}}>
                <div style={{...mono,fontSize:9,color:activeTool===t.id?GOLD:"rgba(247,244,239,0.35)",letterSpacing:"0.08em",textTransform:"uppercase",marginBottom:1}}>{t.num}</div>
                <div style={{...serif,fontSize:12,fontWeight:700,color:activeTool===t.id?S1:"rgba(247,244,239,0.7)",lineHeight:1.25,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{t.label}</div>
                <div style={{fontSize:10,color:activeTool===t.id?"rgba(247,244,239,0.6)":"rgba(247,244,239,0.35)",marginTop:1,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{t.sub}</div>
              </div>
            </button>
          ))}
        </div>
        <div style={{padding:"14px 16px",borderTop:"1px solid rgba(247,244,239,0.08)"}}>
          <div style={{...mono,fontSize:8,color:"rgba(247,244,239,0.3)",letterSpacing:"0.08em",lineHeight:1.9}}>Strategy Made Real<br/>Eight Tools · One Suite</div>
        </div>
      </nav>
      ...
      <div style={{flex:1,overflowY:"auto",background:S1}}>
        ...
        {/* Tool hero */}
        <div style={{padding:"22px 32px 18px",borderBottom:"3px solid "+GOLD,background:S1,flexShrink:0}}>
          <div style={{...mono,fontSize:9,color:G,letterSpacing:"0.18em",textTransform:"uppercase",marginBottom:4,...mono,background:GOLD,display:"inline-block",padding:"2px 8px"}}>Tool {active.num}</div>
          <div style={{...serif,fontSize:"clamp(20px,2.5vw,32px)",fontWeight:700,color:T1,lineHeight:1.15}}>{active.label}</div>
          <div style={{fontSize:13,color:T3,marginTop:4}}>{active.sub}</div>
        </div>

        {/* Tool body */}
        <div style={{padding:"28px 32px 64px",maxWidth:activeTool==="role"?"none":960}}>
