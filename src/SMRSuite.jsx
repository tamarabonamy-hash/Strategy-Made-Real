...
function Callout({children}){return <div style={{background:"rgba(212,168,71,0.07)",border:"1px solid rgba(212,168,71,0.3)",padding:"14px 18px",marginBottom:24,fontSize:13,color:"rgba(44,74,62,0.6)",lineHeight:1.7}}>{children}</div>;}
...
function DataTable({headers,rows}){return(<table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}><thead><tr>{headers.map((h,i)=><th key={i} style={{...mono,fontSize:9,color:"rgba(44,74,62,0.3)",letterSpacing:"0.1em",textTransform:"uppercase",padding:"10px 14px",textAlign:"left",borderBottom:"1px solid rgba(44,74,62,0.1)",background:"rgba(44,74,62,0.06)"}}>{h}</th>)}</tr></thead><tbody>{rows.map((row,ri)=><tr key={ri} style={{background:ri%2===0?"rgba(44,74,62,0.04)":"transparent"}}>{row.map((cell,ci)=><td key={ci} style={{padding:"11px 14px",borderBottom:"1px solid rgba(247,244,239,0.05)",color:"rgba(44,74,62,0.65)",lineHeight:1.5}}>{cell}</td>)}</tr>)}</tbody></table>);}
...
function Tool1Diagnostic(){
  ...
  return(<div>
    ...
    {view==="input"&&(<div>
      ...
      <div style={{marginBottom:24}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}><span style={{...mono,fontSize:10,color:"rgba(44,74,62,0.3)"}}>Scoring progress</span><strong style={{...mono,fontSize:10,color:GOLD}}>{scored} of 6 scored</strong></div><div style={{height:2,background:"rgba(44,74,62,0.1)",borderRadius:1}}><div style={{height:"100%",width:`${(scored/6)*100}%`,background:GOLD,borderRadius:1,transition:"width 0.4s"}}/></div></div>
      {DIAG_DIMS.map(dim=>{const s=scores[dim.id];const col=scoreColor(s);const isOpen=expanded[dim.id];return(<div key={dim.id} style={{background:"rgba(44,74,62,0.05)",border:"1px solid rgba(44,74,62,0.12)",marginBottom:2,overflow:"hidden"}}>
        <div onClick={()=>setExpanded(p=>({...p,[dim.id]:!p[dim.id]}))} style={{padding:"18px 22px",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",gap:12}}>
          <div style={{display:"flex",gap:14,alignItems:"center",flex:1}}><span style={{...mono,fontSize:10,color:"rgba(212,168,71,0.55)"}}>{dim.num}</span><div><div style={{...serif,fontSize:15,fontWeight:700,color:TEXT}}>{dim.label}</div><div style={{fontSize:11,color:"rgba(44,74,62,0.4)",marginTop:2}}>{dim.tagline}</div></div></div>
          <div style={{display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
            {s?<span style={{...mono,fontSize:9,color:col,background:`${col}18`,border:`1px solid ${col}44`,padding:"2px 8px",textTransform:"uppercase"}}>{s} — {SLABELS[s]}</span>:<span style={{...mono,fontSize:9,color:"rgba(44,74,62,0.28)",background:"rgba(44,74,62,0.04)",border:"1px solid rgba(44,74,62,0.18)",padding:"2px 8px"}}>Not scored</span>}
            <span style={{color:"rgba(44,74,62,0.3)",fontSize:10,transform:isOpen?"rotate(180deg)":"none",transition:"transform 0.2s"}}>▼</span>
          </div>
        </div>
        {isOpen&&(<div style={{borderTop:"1px solid rgba(44,74,62,0.1)",padding:"0 22px 20px"}}>
          <div style={{fontSize:12,color:"rgba(44,74,62,0.45)",lineHeight:1.7,padding:"14px 0 16px",borderBottom:"1px solid rgba(247,244,239,0.05)",marginBottom:16,fontStyle:"italic"}}>{dim.intro}</div>
...
function Tool2Prioritisation(){
  ...
  return(<div>
    ...
    {view==="input"&&(<div>
      ...
      <Card style={{padding:22,marginBottom:12}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
          <div style={{gridColumn:"1/-1"}}><div style={{...mono,fontSize:9,color:"rgba(44,74,62,0.4)",letterSpacing:"0.09em",textTransform:"uppercase",marginBottom:4}}>Initiative Name</div><input value={fw} onChange={e=>setFw(e.target.value)} placeholder="e.g. ERP Finance Transformation" style={{width:"100%",background:"rgba(44,74,62,0.05)",border:"1px solid rgba(44,74,62,0.18)",padding:"8px 11px",color:TEXT,...sans,fontSize:13,outline:"none"}}/></div>
          <div><div style={{...mono,fontSize:9,color:"rgba(44,74,62,0.4)",letterSpacing:"0.09em",textTransform:"uppercase",marginBottom:4}}>Owner</div><input value={fo} onChange={e=>setFo(e.target.value)} placeholder="e.g. CFO" style={{width:"100%",background:"rgba(44,74,62,0.05)",border:"1px solid rgba(44,74,62,0.18)",padding:"8px 11px",color:TEXT,...sans,fontSize:13,outline:"none"}}/></div>
          <div><div style={{...mono,fontSize:9,color:"rgba(44,74,62,0.4)",letterSpacing:"0.09em",textTransform:"uppercase",marginBottom:4}}>Duration</div><select value={fd} onChange={e=>setFd(e.target.value)} style={{width:"100%",background:BG,border:"1px solid rgba(44,74,62,0.18)",padding:"8px 11px",color:TEXT,...mono,fontSize:11,outline:"none"}}>{[["1","< 3 months"],["2","3–6 months"],["3","6–12 months"],["4","12–18 months"],["5","18+ months"]].map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:3,marginBottom:14}}>
          {PDIMS.map((dim,di)=>{const col=DCOLS[di];const val=fs[dim.id]||0;return(<div key={dim.id} style={{background:"rgba(44,74,62,0.04)",border:"1px solid rgba(44,74,62,0.1)",padding:"10px 10px 12px"}}>
            ...
            <div style={{fontSize:10,color:"rgba(44,74,62,0.3)",lineHeight:1.4,marginBottom:8}}>{dim.tip}</div>
      ...
      <div style={{...mono,fontSize:10,color:"rgba(44,74,62,0.35)",letterSpacing:"0.1em",marginBottom:10}}>{initiatives.length} initiative{initiatives.length!==1?"s":""} added</div>
      {initiatives.length===0?(<div style={{padding:32,textAlign:"center",border:"1px dashed rgba(44,74,62,0.2)",background:"rgba(44,74,62,0.04)"}}><div style={{...serif,fontSize:14,color:"rgba(44,74,62,0.28)",fontStyle:"italic",marginBottom:4}}>No initiatives added yet</div><div style={{fontSize:11,color:"rgba(44,74,62,0.18)"}}>Add your first above, or load sample data to explore the model</div></div>):<div style={{display:"flex",flexDirection:"column",gap:2,marginBottom:20}}>{initiatives.map(init=>{const tier=getTier(init.weightedScore);return(<div key={init.id} style={{background:"rgba(44,74,62,0.04)",border:"1px solid rgba(44,74,62,0.12)",padding:"12px 16px",display:"flex",alignItems:"center",gap:12}}><div style={{flex:1,...serif,fontSize:13,color:TEXT}}>{init.name}</div><div style={{...mono,fontSize:10,color:"rgba(44,74,62,0.35)"}}>{init.owner}</div><div style={{display:"flex",gap:4}}>{PDIMS.map((d,di)=><div key={d.id} style={{width:20,height:20,display:"flex",alignItems:"center",justifyContent:"center",...mono,fontSize:9,fontWeight:700,background:DCOLS[di]+"22",color:DCOLS[di],border:`1px solid ${DCOLS[di]}44`}}>{init.scores[d.id]}</div>)}</div><div style={{...mono,fontSize:11,color:tier.color,minWidth:40,textAlign:"right"}}>{init.weightedScore.toFixed(2)}</div><button onClick={()=>setInitiatives(p=>p.filter(i=>i.id!==init.id))} style={{background:"none",border:"none",color:"rgba(44,74,62,0.25)",cursor:"pointer",fontSize:13}}>✕</button></div>);})}</div>}
    ...
    {view==="sequence"&&(<div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:2}}>{qs.map((q,qi)=><div key={qi} style={{border:"1px solid rgba(44,74,62,0.12)",overflow:"hidden"}}><div style={{background:"rgba(44,74,62,0.06)",padding:"10px 12px",borderBottom:"1px solid rgba(247,244,239,0.07)"}}><div style={{...mono,fontSize:10,color:"rgba(44,74,62,0.35)",letterSpacing:"0.12em",textTransform:"uppercase"}}>{q.label}</div><div style={{fontSize:11,color:"rgba(44,74,62,0.25)",marginTop:2}}>{q.sub} · {q.items.length} item{q.items.length!==1?"s":""}</div></div><div style={{padding:8,minHeight:80}}>{q.items.length===0?<div style={{...mono,fontSize:9,color:"rgba(44,74,62,0.2)",textAlign:"center",paddingTop:16}}>—</div>:q.items.map(init=>{const tier=getTier(init.weightedScore);return(<div key={init.id} style={{borderLeft:`3px solid ${tier.color}`,background:"rgba(44,74,62,0.06)",padding:"8px 10px",marginBottom:5}}><div style={{...serif,fontSize:11,color:TEXT,marginBottom:2}}>{init.name}</div><div style={{...mono,fontSize:8,color:tier.color}}>{init.owner} · {init.weightedScore.toFixed(2)}</div></div>);})}</div></div>)}</div>
    </div>)}
    {view==="resources"&&(<div>
  ...
  return(<div>
    ...
    {step==="setup"&&(<div>
      <div style={{background:"rgba(44,74,62,0.06)",border:"1px solid rgba(212,168,71,0.15)",padding:24,marginBottom:24}}>
        <div style={{...serif,fontSize:18,fontWeight:700,color:TEXT,marginBottom:8}}>The Mental Model</div>
        <div style={{fontSize:13,color:"rgba(44,74,62,0.5)",lineHeight:1.7,marginBottom:18}}>Most change decisions are held hostage by the wrong question — "should we do this?" The right question is "which version of this gives us the best return on disruption?" The Decision Stack forces three options into the same room and lets the evidence decide.</div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:2}}>{[["A","Full commitment","The complete change. Maximum investment, maximum potential. Name it honestly.",OCOLS[0],OLIGHTS[0]],["B","Partial or phased","Scoped-down or staged. Lower cost and risk — but does it still deliver enough value to justify the disruption?",OCOLS[1],OLIGHTS[1]],["C","Do nothing","Not absence of risk — presence of a different risk. Naming this forces the room to confront what inaction actually costs.",OCOLS[2],OLIGHTS[2]]].map(([letter,t,desc,col,bg])=><div key={letter} style={{background:bg,borderTop:`3px solid ${col}`,padding:"14px 16px"}}><div style={{...mono,fontSize:9,color:col,letterSpacing:"0.12em",textTransform:"uppercase",marginBottom:4}}>Option {letter}</div><div style={{...serif,fontSize:14,fontWeight:700,color:TEXT,marginBottom:6}}>{t}</div><div style={{fontSize:11,color:"rgba(44,74,62,0.45)",lineHeight:1.6}}>{desc}</div></div>)}</div>
      </div>
      <Card style={{padding:22,marginBottom:12}}>
        <SL>Name the Decision</SL>
        <input value={title} onChange={e=>setTitle(e.target.value)} placeholder="e.g. Finance ERP Transformation" style={{width:"100%",background:"rgba(44,74,62,0.05)",border:"1px solid rgba(44,74,62,0.18)",padding:"9px 12px",color:TEXT,...sans,fontSize:13,outline:"none",marginBottom:18}}/>
        <SL>Name the Three Options</SL>
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>{opts.map((opt,i)=><div key={opt}><div style={{...mono,fontSize:9,color:OCOLS[i],letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:4}}>Option {opt.toUpperCase()} — {OLABELS[i]}</div><input value={names[i]} onChange={e=>{const n=[...names];n[i]=e.target.value;setNames(n);}} placeholder={["e.g. Full ERP implementation","e.g. Phase 1 only: GL & AP","e.g. Continue on legacy system"][i]} style={{width:"100%",background:OLIGHTS[i],border:`1px solid ${OCOLS[i]}33`,padding:"8px 11px",color:TEXT,...sans,fontSize:12,outline:"none",marginBottom:6}}/></div>)}</div>
      </Card>
    ...
    {step==="result"&&(<div>
      <div style={{borderLeft:`4px solid ${OCOLS[wi]}`,background:"rgba(44,74,62,0.06)",padding:"24px 28px",marginBottom:2}}><div style={{...mono,fontSize:10,color:OCOLS[wi],letterSpacing:"0.14em",textTransform:"uppercase",marginBottom:6}}>The Stack recommends</div><div style={{...serif,fontSize:24,fontWeight:700,color:TEXT,marginBottom:8}}>{names[wi]||`Option ${winner.toUpperCase()}`}</div><div style={{fontSize:13,color:"rgba(44,74,62,0.55)",lineHeight:1.7,maxWidth:580}}>{winner==="a"?"The full commitment option scores highest across the dimensions that matter. The investment appears proportionate to the strategic return.":winner==="b"?"The phased approach delivers meaningful value at lower risk. It may not capture the full strategic opportunity — but it is the option the organisation can most confidently execute.":"The analysis suggests that neither current change option clears the bar. The case for change needs strengthening before a resource commitment is made."}</div></div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:2,marginBottom:2}}>{opts.map((opt,i)=>{const col=OCOLS[i];const score=ws[opt];const isW=opt===winner;return(<div key={opt} style={{background:"rgba(44,74,62,0.05)",border:`1px solid ${isW?"rgba(212,168,71,0.3)":"rgba(247,244,239,0.07)"}`,padding:22,position:"relative"}}>{isW&&<div style={{position:"absolute",top:-1,right:14,background:GOLD,color:G,...mono,fontSize:8,letterSpacing:"0.1em",textTransform:"uppercase",padding:"2px 7px"}}>Recommended</div>}<div style={{...mono,fontSize:9,color:col,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:3}}>Option {opt.toUpperCase()}</div><div style={{...serif,fontSize:14,fontWeight:700,color:TEXT,marginBottom:14}}>{names[i]||OLABELS[i]}</div><div style={{...serif,fontSize:44,fontWeight:700,color:col,lineHeight:1}}>{score.toFixed(2)}</div><div style={{...mono,fontSize:9,color:"rgba(44,74,62,0.35)",letterSpacing:"0.08em",marginBottom:16}}>/ 5.0</div>{DDIMS.map(d=>{const val=scores[opt][d.id]||0;return(<div key={d.id} style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}><span style={{fontSize:10,color:"rgba(44,74,62,0.4)",flex:1}}>{d.label}</span><div style={{width:50,height:2,background:"rgba(247,244,239,0.08)"}}><div style={{height:"100%",width:`${(val/5)*100}%`,background:`${col}88`,borderBottom:`1.5px solid ${col}`}}/></div><span style={{...mono,fontSize:10,color:col,minWidth:12}}>{val}</span></div>);})}</div>);})}
      </div>
      <Card style={{padding:24,display:"flex",flexDirection:"column",alignItems:"center",marginBottom:2}}><SL>Dimension Profile — All Three Options</SL><svg width="300" height="300" viewBox="0 0 320 320">{buildRadar()}</svg><div style={{display:"flex",gap:18,marginTop:10}}>{opts.map((opt,i)=><div key={opt} style={{display:"flex",alignItems:"center",gap:6,...mono,fontSize:9,color:"rgba(44,74,62,0.45)"}}><div style={{width:8,height:8,borderRadius:"50%",background:OCOLS[i]}}/>{names[i]||`Option ${opt.toUpperCase()}`}</div>)}</div></Card>
      <Card style={{padding:24,marginBottom:2}}><SL>Before You Decide</SL><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>{[[OCOLS[0],"If you choose Option A",`${names[0]||"Option A"} — What does success look like in 12 months? Who owns the decision to stop if delivery fails?`],[OCOLS[1],"If you choose Option B",`${names[1]||"Option B"} — Is this genuinely strategic or a way to avoid the harder conversation? When is the decision point to scale up?`],[OCOLS[2],"If you choose Option C",`${names[2]||"Option C"} — What is the cost of inaction compounding over 12–24 months? Is this a decision to stop, or a decision to wait?`],["rgba(247,244,239,0.3)","Before any option","What assumption, if proven wrong, would change this decision? Who needs to own this decision — not just approve it? What would you need to see in 90 days to confirm you chose correctly?"]].map(([col,label,text])=><div key={label} style={{borderLeft:`3px solid ${col}`,background:"rgba(44,74,62,0.03)",padding:"14px 16px"}}><div style={{...mono,fontSize:9,color:col,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:6}}>{label}</div><div style={{fontSize:12,color:"rgba(44,74,62,0.45)",lineHeight:1.6}}>{text}</div></div>)}</div></Card>
      <div style={{background:"rgba(212,168,71,0.07)",border:"1px solid rgba(212,168,71,0.3)",padding:24,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:16}}><div><div style={{...serif,fontSize:16,color:TEXT,marginBottom:4}}>Turn this into a decision your leadership team owns.</div><div style={{fontSize:12,color:"rgba(44,74,62,0.4)"}}>Strategy Made Real facilitates structured decision sessions that move from The Stack to a committed, documented decision in a single session.</div></div><div style={{display:"flex",gap:10,flexWrap:"wrap"}}><GBtn onClick={()=>exportDecisionStack({title,names,scores,ws,winner,dims:DDIMS})}>↓ Download Report</GBtn><OBtn onClick={()=>setStep("setup")}>← Start Over</OBtn></div></div>
    </div>)}
  </div>);
}
...
function Tool4RoleAnalyser(){
  ...
  const sections=result?[
    {id:"1",num:"01",title:"Role Context & Purpose",content:(<div>
      ...
      <div style={{marginBottom:10}}><div style={{...mono,fontSize:9,color:GOLD,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:6}}>Organisation Purpose</div><div style={{fontSize:13,color:"rgba(44,74,62,0.75)",lineHeight:1.6}}>{result.purposeAndStrategy?.organisationPurpose}</div></div>
      <div><div style={{...mono,fontSize:9,color:GOLD,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:6}}>Strategic Contribution</div><div style={{fontSize:13,color:"rgba(44,74,62,0.75)",lineHeight:1.6}}>{result.purposeAndStrategy?.roleContribution}</div></div>
    </div>)},
    {id:"2",num:"02",title:"Role Success Statement",content:(<div>
      <div style={{background:"rgba(44,74,62,0.05)",border:"1px solid rgba(212,168,71,0.15)",padding:22,marginBottom:14}}><div style={{...serif,fontSize:14,color:TEXT,lineHeight:1.7}}>{result.roleSuccessStatement}</div></div>
      <div style={{background:"rgba(44,74,62,0.04)",border:"1px solid rgba(44,74,62,0.12)",padding:16}}><div style={{...mono,fontSize:9,color:GOLD,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:8}}>Success in This Business</div><div style={{fontSize:12,color:"rgba(44,74,62,0.65)",lineHeight:1.7}}>{result.successInBusiness}</div></div>
    </div>)},
    ...
    {id:"4",num:"04",title:"Responsibilities by Pillar",content:(<div style={{display:"flex",flexDirection:"column",gap:2}}>
      {result.responsibilitiesByPillar?.map((pillar,i)=><div key={i} style={{background:"rgba(44,74,62,0.04)",border:"1px solid rgba(44,74,62,0.12)",padding:"16px 20px",borderLeft:`3px solid ${GOLD}`}}>
        <div style={{...serif,fontSize:14,fontWeight:700,color:TEXT,marginBottom:4}}>{pillar.pillar}</div>
        <div style={{fontSize:12,color:"rgba(44,74,62,0.5)",fontStyle:"italic",lineHeight:1.5,marginBottom:10}}>{pillar.purpose}</div>
        <ul style={{listStyle:"none"}}>{pillar.responsibilities?.map((r,j)=><li key={j} style={{fontSize:12,color:"rgba(44,74,62,0.65)",lineHeight:1.5,paddingLeft:14,position:"relative",marginBottom:5}}><span style={{position:"absolute",left:0,color:GOLD}}>—</span>{r}</li>)}</ul>
      </div>)}
    </div>)},
    {id:"5",num:"05",title:"Key Competencies",content:(<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
      {result.keyCompetencies?.map((c,i)=><div key={i} style={{background:"rgba(44,74,62,0.04)",border:"1px solid rgba(44,74,62,0.12)",padding:"12px 14px",display:"flex",gap:10,alignItems:"flex-start"}}>
        ...
        <div><div style={{...serif,fontSize:13,fontWeight:700,color:TEXT,marginBottom:3}}>{c.name}</div><div style={{fontSize:12,color:"rgba(44,74,62,0.5)",lineHeight:1.5}}>{c.description}</div></div>
      </div>)}
    </div>)},
    {id:"6",num:"06",title:"What Success Looks Like",content:(<div>
      {result.whatSuccessLooksLike?.map((item,i)=><div key={i} style={{marginBottom:16,background:"rgba(44,74,62,0.04)",border:"1px solid rgba(44,74,62,0.12)",padding:"14px 18px"}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:8}}>
          ...
          <div><div style={{...mono,fontSize:9,color:GOLD,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:4}}>Purpose</div><div style={{fontSize:12,color:"rgba(44,74,62,0.65)",lineHeight:1.5}}>{item.purpose}</div></div>
        </div>
        <div style={{...mono,fontSize:9,color:"rgba(44,74,62,0.4)",letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:6}}>Key Activities & Measures</div>
        <ul style={{listStyle:"none"}}>{item.keyActivities?.map((a,j)=><li key={j} style={{fontSize:12,color:"rgba(44,74,62,0.65)",lineHeight:1.5,paddingLeft:14,position:"relative",marginBottom:4}}><span style={{position:"absolute",left:0,color:GOLD}}>—</span>{a}</li>)}</ul>
      </div>)}
    </div>)},
    {id:"7",num:"07",title:"Circle of Influence",content:(<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
      {[["Key Internal Relationships",result.circleOfInfluence?.internal,GOLD],["Key External Relationships",result.circleOfInfluence?.external,"#3a6b8a"]].map(([label,items,col])=><div key={label} style={{background:"rgba(44,74,62,0.04)",border:"1px solid rgba(44,74,62,0.12)",borderTop:`3px solid ${col}`,padding:16}}><div style={{...mono,fontSize:9,color:col,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:10}}>{label}</div>{items?.map((item,i)=><div key={i} style={{fontSize:12,color:"rgba(44,74,62,0.65)",lineHeight:1.5,paddingLeft:12,position:"relative",marginBottom:4}}><span style={{position:"absolute",left:0,color:col,fontSize:10}}>—</span>{item}</div>)}</div>)}
    </div>)},
    {id:"8",num:"08",title:"Values & Behaviour Alignment",content:(<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
      {result.valuesAndBehaviours?.map((v,i)=><div key={i} style={{background:"rgba(44,74,62,0.04)",border:"1px solid rgba(44,74,62,0.12)",padding:"12px 14px"}}><div style={{...serif,fontSize:13,fontWeight:700,color:TEXT,marginBottom:4}}>{v.value}</div><div style={{fontSize:12,color:"rgba(44,74,62,0.55)",lineHeight:1.5}}>{v.description}</div></div>)}
    </div>)},
    {id:"9",num:"09",title:"Time-Based Success Metrics",content:(<div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
      {[["Quarterly Priorities",result.timeBasedMetrics?.quarterly,GOLD],["6-Month Goals",result.timeBasedMetrics?.sixMonth,G_MID],["Annual Measures",result.timeBasedMetrics?.annual,"#3a6b8a"]].map(([label,items,col])=><div key={label} style={{background:"rgba(44,74,62,0.04)",border:"1px solid rgba(44,74,62,0.12)",borderTop:`3px solid ${col}`,padding:16}}><div style={{...mono,fontSize:9,color:col,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:10}}>{label}</div>{items?.map((item,i)=><div key={i} style={{display:"flex",gap:8,alignItems:"flex-start",marginBottom:6}}><span style={{...mono,fontSize:10,color:col,fontWeight:700,flexShrink:0}}>{i+1}</span><span style={{fontSize:12,color:"rgba(44,74,62,0.65)",lineHeight:1.5}}>{item}</span></div>)}</div>)}
    </div>)},
  ]:[];
  return(<div style={{display:"grid",gridTemplateColumns:"300px 1fr",gap:0,minHeight:600}}>
    <div style={{borderRight:"1px solid rgba(44,74,62,0.12)",padding:"24px 20px",background:"rgba(44,74,62,0.04)"}}>
      <SL>Role Details</SL>
      {[["role","Role Title *","e.g. Head of Customer Experience"],["org","Organisation *","e.g. Regional Bank"],["level","Level / Seniority","e.g. Senior Manager"],["reports","Direct Reports","e.g. 4 direct, 12 total"]].map(([key,label,ph])=><div key={key} style={{marginBottom:10}}><div style={{...mono,fontSize:9,color:"rgba(44,74,62,0.4)",letterSpacing:"0.09em",textTransform:"uppercase",marginBottom:4}}>{label}</div><input value={form[key]} onChange={e=>setForm(p=>({...p,[key]:e.target.value}))} placeholder={ph} style={{width:"100%",background:"rgba(44,74,62,0.05)",border:"1px solid rgba(44,74,62,0.18)",padding:"8px 11px",color:TEXT,...sans,fontSize:12,outline:"none"}}/></div>)}
      <div style={{height:1,background:"rgba(247,244,239,0.07)",margin:"16px 0"}}/>
      <SL>Context</SL>
      {[["context","Organisational Context","What is happening right now? Key priorities, changes underway, pressures..."],["pillars","Strategic Pillars","e.g. Customer first, operational excellence, digital transformation"],["challenges","Key Challenges","What are the biggest challenges this role needs to solve?"]].map(([key,label,ph])=><div key={key} style={{marginBottom:10}}><div style={{...mono,fontSize:9,color:"rgba(44,74,62,0.4)",letterSpacing:"0.09em",textTransform:"uppercase",marginBottom:4}}>{label}</div><textarea value={form[key]} onChange={e=>setForm(p=>({...p,[key]:e.target.value}))} placeholder={ph} rows={3} style={{width:"100%",background:"rgba(44,74,62,0.05)",border:"1px solid rgba(44,74,62,0.18)",padding:"8px 11px",color:TEXT,...sans,fontSize:12,outline:"none",resize:"vertical",lineHeight:1.5}}/></div>)}
      ...
      {result&&<button onClick={()=>setResult(null)} style={{width:"100%",background:"transparent",border:"1px solid rgba(44,74,62,0.18)",color:"rgba(44,74,62,0.3)",padding:"9px",...mono,fontSize:10,letterSpacing:"0.1em",textTransform:"uppercase",cursor:"pointer",marginTop:8}}>Clear & Start Over</button>}
    </div>
    <div style={{padding:"24px 28px"}}>
      {loading&&<div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:400,textAlign:"center"}}><div style={{width:44,height:44,border:"3px solid rgba(212,168,71,0.15)",borderTop:`3px solid ${GOLD}`,borderRadius:"50%",animation:"spin 1s linear infinite",margin:"0 auto 18px"}}/><div style={{...mono,fontSize:11,color:"rgba(44,74,62,0.4)",letterSpacing:"0.1em",textTransform:"uppercase"}}>Analysing role...</div><div style={{fontSize:12,color:"rgba(44,74,62,0.28)",marginTop:6,fontStyle:"italic"}}>Building your strategy playbook</div></div>}
      {error&&<div style={{background:"rgba(185,64,64,0.1)",border:"1px solid rgba(185,64,64,0.25)",padding:18,fontSize:13,color:"rgba(44,74,62,0.6)",lineHeight:1.6}}>{error}</div>}
      {!loading&&!result&&!error&&<div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:400,textAlign:"center"}}><div style={{opacity:0.1,marginBottom:20}}><div style={{width:60,height:60,borderRadius:"50%",background:"rgba(212,168,71,0.1)",border:"2px solid rgba(212,168,71,0.2)"}}/></div><div style={{...serif,fontSize:20,color:"rgba(44,74,62,0.35)",marginBottom:6}}>Enter role details to begin</div><div style={{fontSize:12,color:"rgba(44,74,62,0.25)",lineHeight:1.6,maxWidth:280}}>Fill in the role title and organisation, add context, and generate a full role and strategy analysis.</div></div>}
      {result&&<div>
        <div style={{marginBottom:24,paddingBottom:18,borderBottom:"1px solid rgba(44,74,62,0.1)"}}><div style={{...serif,fontSize:26,fontWeight:700,color:TEXT,marginBottom:4}}>{form.role}</div><div style={{display:"flex",gap:10,flexWrap:"wrap",marginTop:8}}>{[form.org,form.level,form.reports].filter(Boolean).map((m,i)=><span key={i} style={{...mono,fontSize:10,color:"rgba(44,74,62,0.45)",background:"rgba(44,74,62,0.06)",border:"1px solid rgba(44,74,62,0.18)",padding:"2px 9px"}}>{m}</span>)}</div></div>
        {sections.map(sec=><div key={sec.id} style={{marginBottom:2,background:"rgba(44,74,62,0.04)",border:"1px solid rgba(44,74,62,0.1)"}}><div onClick={()=>setOpen(p=>({...p,[sec.id]:!p[sec.id]}))} style={{padding:"14px 18px",cursor:"pointer",display:"flex",alignItems:"center",gap:10}}><span style={{...mono,fontSize:10,color:GOLD}}>{sec.num}</span><span style={{...serif,fontSize:15,fontWeight:700,color:TEXT}}>{sec.title}</span><span style={{marginLeft:"auto",color:"rgba(44,74,62,0.3)",fontSize:10,transform:open[sec.id]?"rotate(180deg)":"none",transition:"transform 0.2s"}}>▼</span></div>{open[sec.id]&&<div style={{padding:"0 18px 18px"}}>{sec.content}</div>}</div>)}
      ...
      {view === "builder" && (
        <div>
          ...
          <div style={{marginBottom:16}}>
            <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:"rgba(44,74,62,0.4)",letterSpacing:"0.09em",textTransform:"uppercase",marginBottom:4}}>Organisation / Programme Name</div>
            <input value={orgName} onChange={e=>setOrgName(e.target.value)} placeholder="e.g. Customer Transformation Programme" style={{background:"rgba(44,74,62,0.05)",border:"1px solid rgba(44,74,62,0.18)",padding:"8px 12px",color:TEXT,fontFamily:"'DM Sans',sans-serif",fontSize:13,outline:"none",width:400}} />
          </div>
            ...
            {MEETING_TYPES.map(m => {
              ...
              return (
                <div key={m.id} onClick={() => toggleMeeting(m.id)} style={{
                  ...
                  <div>
                    <div style={{fontFamily:"'Playfair Display',serif",fontSize:14,fontWeight:700,color:TEXT,marginBottom:2}}>{m.label}</div>
                    <div style={{fontSize:11,color:"rgba(44,74,62,0.45)",lineHeight:1.5}}>{m.purpose}</div>
                  </div>
                  ...
                  <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:"rgba(44,74,62,0.35)",whiteSpace:"nowrap"}}>{m.duration}</div>
                  <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:"rgba(44,74,62,0.35)",whiteSpace:"nowrap"}}>{m.owner}</div>
                </div>
              );
            })}
          </div>
          ...
          {customMeetings.map(m => (
            <div key={m.id} style={{background:"rgba(44,74,62,0.03)",border:"1px solid rgba(212,168,71,0.15)",padding:"12px 18px",display:"flex",alignItems:"center",gap:12,marginBottom:2}}>
              ...
              <div style={{flex:1}}>
                ...
                <div style={{fontSize:11,color:"rgba(44,74,62,0.4)"}}>{m.cadence} · {m.duration} · {m.owner}</div>
              </div>
              <button onClick={() => setCustomMeetings(p => p.filter(x => x.id!==m.id))} style={{background:"none",border:"none",color:"rgba(44,74,62,0.25)",cursor:"pointer",fontSize:13}}>✕</button>
            </div>
          ))}
          ...
          {addingCustom ? (
            <div style={{background:"rgba(44,74,62,0.06)",border:"1px solid rgba(212,168,71,0.2)",padding:20,marginTop:8}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
                {[["label","Meeting Name","e.g. Change Champion Sync"],["owner","Owner","e.g. Change Lead"],["attendees","Attendees","e.g. Change champions, team leads"],["duration","Duration","e.g. 45 min"]].map(([key,label,ph]) => (
                  <div key={key}>
                    <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:"rgba(44,74,62,0.4)",letterSpacing:"0.09em",textTransform:"uppercase",marginBottom:4}}>{label}</div>
                    <input value={customForm[key]} onChange={e => setCustomForm(p=>({...p,[key]:e.target.value}))} placeholder={ph} style={{width:"100%",background:"rgba(44,74,62,0.05)",border:"1px solid rgba(44,74,62,0.18)",padding:"7px 10px",color:TEXT,fontFamily:"'DM Sans',sans-serif",fontSize:12,outline:"none"}} />
                  </div>
                ))}
                <div style={{gridColumn:"1/-1"}}>
                  <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:"rgba(44,74,62,0.4)",letterSpacing:"0.09em",textTransform:"uppercase",marginBottom:4}}>Purpose</div>
                  <input value={customForm.purpose} onChange={e => setCustomForm(p=>({...p,purpose:e.target.value}))} placeholder="What does this meeting exist to do?" style={{width:"100%",background:"rgba(44,74,62,0.05)",border:"1px solid rgba(44,74,62,0.18)",padding:"7px 10px",color:TEXT,fontFamily:"'DM Sans',sans-serif",fontSize:12,outline:"none"}} />
                </div>
                <div>
                  <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:"rgba(44,74,62,0.4)",letterSpacing:"0.09em",textTransform:"uppercase",marginBottom:4}}>Cadence</div>
                  <select value={customForm.cadence} onChange={e=>setCustomForm(p=>({...p,cadence:e.target.value}))} style={{width:"100%",background:"#F7F4EF",border:"1px solid rgba(44,74,62,0.18)",padding:"7px 10px",color:TEXT,fontFamily:"'DM Mono',monospace",fontSize:11,outline:"none"}}>
            ...
            <button onClick={() => setAddingCustom(true)} style={{background:"transparent",border:"1px dashed rgba(44,74,62,0.25)",color:"rgba(44,74,62,0.35)",padding:"10px 18px",fontFamily:"'DM Mono',monospace",fontSize:10,letterSpacing:"0.1em",textTransform:"uppercase",cursor:"pointer",marginTop:8}}>+ Add Custom Meeting</button>
          )}
      ...
      {view === "calendar" && (
        <div>
          <div style={{marginBottom:20}}>
            <div style={{fontFamily:"'Playfair Display',serif",fontSize:20,fontWeight:700,color:TEXT,marginBottom:4}}>{orgName || "Programme"} — Operating Rhythm</div>
            <div style={{fontSize:12,color:"rgba(44,74,62,0.4)"}}>Weekly recurring meetings shown below. Monthly and quarterly cadences listed separately.</div>
          </div>
            ...
            {WEEK_DAYS.map(day => (
              <div key={day} style={{border:"1px solid rgba(44,74,62,0.12)",overflow:"hidden"}}>
                <div style={{background:"rgba(44,74,62,0.08)",padding:"10px 12px",borderBottom:"1px solid rgba(247,244,239,0.07)"}}>
                  <div style={{fontFamily:"'DM Mono',monospace",fontSize:11,color:"rgba(44,74,62,0.5)",letterSpacing:"0.12em",textTransform:"uppercase"}}>{day}</div>
                </div>
                  ...
                  {byDay[day].filter((m,i,arr) => arr.findIndex(x=>x.id===m.id)===i).map(m => {
                    ...
                    return (
                      <div key={m.id} style={{borderLeft:`3px solid ${col}`,background:"rgba(44,74,62,0.06)",padding:"8px 10px",marginBottom:5}}>
                        <div style={{fontFamily:"'Playfair Display',serif",fontSize:11,color:TEXT,marginBottom:2}}>{m.label}</div>
                        <div style={{fontFamily:"'DM Mono',monospace",fontSize:8,color:col}}>{m.duration}</div>
                      </div>
                    );
                  })}
                  {byDay[day].length === 0 && <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:"rgba(44,74,62,0.18)",textAlign:"center",paddingTop:16}}>—</div>}
                </div>
              </div>
            ))}
          </div>
                ...
                {monthlyCadence.map(m => {
                  ...
                  return (
                    <div key={m.id} style={{display:"grid",gridTemplateColumns:"120px 1fr auto",gap:14,alignItems:"center",padding:"10px 14px",background:"rgba(44,74,62,0.04)",border:`1px solid rgba(247,244,239,0.07)`}}>
                      ...
                      <div>
                        <div style={{fontFamily:"'Playfair Display',serif",fontSize:13,color:TEXT,marginBottom:2}}>{m.label}</div>
                        <div style={{fontSize:11,color:"rgba(44,74,62,0.4)"}}>{m.purpose}</div>
                      </div>
                      <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:"rgba(44,74,62,0.35)",textAlign:"right"}}>
                        <div>{m.duration}</div>
      ...
      {view === "register" && (
        <div>
          <Card style={{padding:0,overflow:"hidden"}}>
            <DataTable
              ...
              rows={allMeetings.map(m => [
                <span style={{fontFamily:"'Playfair Display',serif",fontSize:13,color:TEXT,fontWeight:700}}>{m.label}</span>,
          ...
          {allMeetings.length === 0 && (
            <div style={{textAlign:"center",padding:40,color:"rgba(44,74,62,0.3)",fontStyle:"italic",fontSize:13}}>No meetings selected. Go to the Builder tab to add meetings.</div>
          )}
        </div>
      )}
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:2}}>
            {[
              ...
              <div key={title} style={{background:"rgba(44,74,62,0.04)",border:"1px solid rgba(44,74,62,0.12)",borderTop:`3px solid ${col}`,padding:20}}>
                <div style={{fontFamily:"'Playfair Display',serif",fontSize:14,fontWeight:700,color:TEXT,marginBottom:8}}>{title}</div>
                <div style={{fontSize:12,color:"rgba(44,74,62,0.55)",lineHeight:1.7}}>{text}</div>
              </div>
            ))}
          </div>
        </div>
    ...
    <div>
      ...
      {view === "builder" && (
        <div>
            ...
            {KPI_TEMPLATES[activePillar].map((kpi, idx) => {
              ...
              return (
                <div key={idx} onClick={() => toggleKPI(activePillar, idx)} style={{
                  ...
                  <div>
                    <div style={{fontFamily:"'Playfair Display',serif",fontSize:13,fontWeight:700,color:TEXT,marginBottom:3}}>{kpi.name}</div>
                    <div style={{fontSize:11,color:"rgba(44,74,62,0.4)",fontFamily:"'DM Mono',monospace"}}>Target: {kpi.target}</div>
                  </div>
                  <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:"rgba(44,74,62,0.35)",whiteSpace:"nowrap"}}>{kpi.frequency}</div>
                ...
                return (
                  <div key={k.id} style={{background:"rgba(44,74,62,0.03)",border:"1px solid rgba(212,168,71,0.15)",padding:"12px 16px",display:"grid",gridTemplateColumns:"26px 1fr auto",gap:12,alignItems:"center",marginBottom:2}}>
                    ...
                    <div>
                      ...
                      <div style={{fontSize:11,color:"rgba(44,74,62,0.4)"}}>{k.pillar} · {k.frequency} · Target: {k.target}</div>
                    </div>
                    <button onClick={() => setCustomKPIs(p=>p.filter(x=>x.id!==k.id))} style={{background:"none",border:"none",color:"rgba(44,74,62,0.25)",cursor:"pointer",fontSize:13}}>✕</button>
                  </div>
                );
              })}
            </div>
          ...
          {addingCustom ? (
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
                {[["name","KPI Name","e.g. Supplier on-time delivery rate"],["formula","Formula / Calculation","e.g. On-time deliveries ÷ Total deliveries × 100"],["target","Target","e.g. ≥ 95%"],["frequency","Measurement Frequency","e.g. Weekly"]].map(([key,label,ph]) => (
                  <div key={key} style={{gridColumn:key==="name"||key==="formula"?"1/-1":"auto"}}>
                    <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:"rgba(44,74,62,0.4)",letterSpacing:"0.09em",textTransform:"uppercase",marginBottom:4}}>{label}</div>
                    <input value={cForm[key]} onChange={e=>setCForm(p=>({...p,[key]:e.target.value}))} placeholder={ph} style={{width:"100%",background:"rgba(44,74,62,0.05)",border:"1px solid rgba(44,74,62,0.18)",padding:"7px 10px",color:TEXT,fontFamily:"'DM Sans',sans-serif",fontSize:12,outline:"none"}} />
                  </div>
                ))}
                <div>
                  <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:"rgba(44,74,62,0.4)",letterSpacing:"0.09em",textTransform:"uppercase",marginBottom:4}}>Pillar</div>
                  <select value={cForm.pillar} onChange={e=>setCForm(p=>({...p,pillar:e.target.value}))} style={{width:"100%",background:"#F7F4EF",border:"1px solid rgba(44,74,62,0.18)",padding:"7px 10px",color:TEXT,fontFamily:"'DM Mono',monospace",fontSize:11,outline:"none"}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:10,paddingTop:20}}>
                  ...
                  <label htmlFor="lead-toggle" style={{fontSize:12,color:"rgba(44,74,62,0.5)",cursor:"pointer"}}>Lead indicator (predictive)</label>
                </div>
              </div>
              <div style={{display:"flex",gap:8}}>
            ...
            <button onClick={() => setAddingCustom(true)} style={{background:"transparent",border:"1px dashed rgba(44,74,62,0.25)",color:"rgba(44,74,62,0.35)",padding:"10px 18px",fontFamily:"'DM Mono',monospace",fontSize:10,letterSpacing:"0.1em",textTransform:"uppercase",cursor:"pointer",marginTop:8}}>+ Add Custom KPI</button>
          )}
      ...
      {view === "scorecard" && (
        <div>
                ...
                <Card style={{padding:0,overflow:"hidden"}}>
                  <DataTable
                    ...
                    rows={pillKPIs.map(k => [
                      ...
                      <span style={{fontFamily:"'DM Mono',monospace",fontSize:10,color:"rgba(44,74,62,0.35)"}}>{k.formula}</span>,
                    ])}
                  />
                </Card>
              </div>
          ...
          {allSelected.length === 0 && <div style={{textAlign:"center",padding:40,color:"rgba(44,74,62,0.3)",fontStyle:"italic",fontSize:13}}>No KPIs selected. Go to the Builder tab to add KPIs.</div>}
        </div>
      )}
        ...
        <div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            <div>
              ...
              {leadKPIs.length === 0 ? <div style={{fontSize:12,color:"rgba(44,74,62,0.28)",fontStyle:"italic",padding:"20px 0"}}>No lead indicators selected yet. Add KPIs marked as lead indicators.</div> :
                leadKPIs.map((k,i) => (
                  <div key={i} style={{background:"rgba(74,122,104,0.06)",border:"1px solid rgba(74,122,104,0.15)",borderLeft:`3px solid ${G_MID}`,padding:"12px 14px",marginBottom:6}}>
                    <div style={{fontFamily:"'Playfair Display',serif",fontSize:13,color:TEXT,marginBottom:3}}>{k.name}</div>
                    <div style={{fontSize:11,color:"rgba(44,74,62,0.45)"}}>{k.frequency} · Target: {k.target}</div>
                  </div>
                ))
              }
            </div>
            <div>
              ...
              {lagKPIs.length === 0 ? <div style={{fontSize:12,color:"rgba(44,74,62,0.28)",fontStyle:"italic",padding:"20px 0"}}>No lag indicators selected yet.</div> :
                lagKPIs.map((k,i) => (
            ...
            {CAP_DOMAINS.map(d => {
              ...
              return (
                <div key={d.id} onClick={() => setActiveDomain(d.id)} style={{
                  ...
                  <div style={{fontFamily:"'Playfair Display',serif",fontSize:13,color:TEXT,marginBottom:4}}>{d.label}</div>
          ...
          {(() => {
            ...
            return (
              <Card style={{padding:24}}>
                <div style={{fontFamily:"'Playfair Display',serif",fontSize:18,fontWeight:700,color:TEXT,marginBottom:6}}>{dom.label}</div>
                <div style={{fontSize:12,color:"rgba(44,74,62,0.5)",lineHeight:1.6,marginBottom:20,fontStyle:"italic"}}>{dom.desc}</div>
                    ...
                    {CAP_LEVELS.map(lvl => {
                      ...
                      return (
                        <div key={lvl.score} onClick={() => setScores(p => ({...p,[dom.id]:lvl.score}))} style={{
                          ...
                          <div style={{fontSize:10,color:"rgba(44,74,62,0.45)",lineHeight:1.5}}>{lvl.desc}</div>
                        </div>
                      );
                    })}
                  </div>
                ...
                <div>
                  <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:"rgba(44,74,62,0.4)",letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:6}}>Notes (optional)</div>
                  <textarea value={notes[dom.id]||""} onChange={e=>setNotes(p=>({...p,[dom.id]:e.target.value}))} placeholder="What is driving this score? What specific evidence or examples?" rows={2} style={{width:"100%",background:"rgba(44,74,62,0.05)",border:"1px solid rgba(44,74,62,0.18)",padding:"8px 11px",color:TEXT,fontFamily:"'DM Sans',sans-serif",fontSize:12,outline:"none",resize:"vertical",lineHeight:1.5}} />
                </div>
              </Card>
            );
          })()}
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:2,marginBottom:24}}>
            {[
              ...
              <Card key={i} style={{padding:20}}>
                ...
                <div style={{fontSize:11,color:"rgba(44,74,62,0.4)",marginTop:4}}>{s.sub}</div>
              </Card>
            ))}
          </div>
                  ...
                  <div key={dom.id} style={{display:"grid",gridTemplateColumns:"180px 1fr 60px 100px",gap:12,alignItems:"center",padding:"8px 0",borderBottom:"1px solid rgba(247,244,239,0.05)"}}>
                    ...
                    <div>
                      <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                        <span style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:"rgba(44,74,62,0.3)"}}>Capability</span>
                    ...
                    <div style={{textAlign:"center"}}>
                      <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:"rgba(44,74,62,0.3)",marginBottom:2}}>Imp.</div>
                    ...
                    <div style={{textAlign:"right"}}>
                      ...
                      ) : <span style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:"rgba(44,74,62,0.25)"}}>Not assessed</span>}
                    </div>
                  </div>
                );
              })}
      ...
      {view === "priorities" && (
        <div>
          {gaps.length === 0 && critGaps.length === 0 ? (
            <div style={{textAlign:"center",padding:48,color:"rgba(44,74,62,0.3)",fontStyle:"italic",fontSize:13}}>No high-priority gaps identified. Assess all domains first, or your organisation is in strong shape.</div>
          ) : (
            <>
              {critGaps.length > 0 && (
                <div style={{background:"rgba(185,64,64,0.06)",border:"1px solid rgba(185,64,64,0.25)",padding:24,marginBottom:12}}>
                  ...
                  {critGaps.map(dom => (
                    <div key={dom.id} style={{borderLeft:`2px solid ${RED}`,paddingLeft:16,marginBottom:16}}>
                      <div style={{fontFamily:"'Playfair Display',serif",fontSize:14,color:TEXT,marginBottom:4}}>{dom.label}</div>
                      <div style={{fontSize:12,color:"rgba(44,74,62,0.5)",lineHeight:1.6,marginBottom:4}}>{dom.desc}</div>
                ...
                <Card style={{padding:24,marginBottom:12}}>
                  ...
                  {gaps.filter(d=>scores[d.id]===2).map(dom => (
                    <div key={dom.id} style={{borderLeft:`2px solid #C0622A`,paddingLeft:16,marginBottom:14}}>
                      <div style={{fontFamily:"'Playfair Display',serif",fontSize:14,color:TEXT,marginBottom:3}}>{dom.label}</div>
                      <div style={{fontSize:12,color:"rgba(44,74,62,0.5)",lineHeight:1.6}}>{dom.desc}</div>
                    </div>
                  ))}
                </Card>
              )}
          ...
          {CAP_DOMAINS.filter(d => scores[d.id] > 0 && scores[d.id] < 4).map(dom => {
            ...
            return (
              <div key={dom.id} style={{background:"rgba(44,74,62,0.04)",border:"1px solid rgba(44,74,62,0.12)",borderLeft:`3px solid ${col}`,padding:"16px 20px",marginBottom:2}}>
                ...
                <div style={{fontSize:12,color:"rgba(44,74,62,0.45)",lineHeight:1.6}}>{dom.desc}</div>
              </div>
            );
          })}
          {CAP_DOMAINS.filter(d => scores[d.id] > 0 && scores[d.id] < 4).length === 0 && (
            <div style={{textAlign:"center",padding:48,color:"rgba(44,74,62,0.3)",fontStyle:"italic",fontSize:13}}>No gaps to action. All assessed domains are at Capable or Strong.</div>
          )}
        </div>
      )}
    </div>
      ...
      {view === "setup" && (
        <div>
          ...
          <Card style={{padding:24,marginBottom:12}}>
            ...
            <div style={{marginBottom:12}}>
              <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:"rgba(44,74,62,0.4)",letterSpacing:"0.09em",textTransform:"uppercase",marginBottom:4}}>Change / Initiative Name</div>
              <input value={changeName} onChange={e=>setChangeName(e.target.value)} placeholder="e.g. Customer Service Process Redesign" style={{width:"100%",background:"rgba(44,74,62,0.05)",border:"1px solid rgba(44,74,62,0.18)",padding:"9px 12px",color:TEXT,fontFamily:"'DM Sans',sans-serif",fontSize:13,outline:"none"}} />
            </div>
            <div style={{marginBottom:12}}>
              <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:"rgba(44,74,62,0.4)",letterSpacing:"0.09em",textTransform:"uppercase",marginBottom:8}}>Type of Change</div>
            ...
            <div style={{marginBottom:12}}>
              <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:"rgba(44,74,62,0.4)",letterSpacing:"0.09em",textTransform:"uppercase",marginBottom:4}}>Impacted Groups</div>
              <textarea value={impactedGroups} onChange={e=>setImpactedGroups(e.target.value)} placeholder="e.g. Customer service team (45 people), Team leaders (8), Operations manager" rows={2} style={{width:"100%",background:"rgba(44,74,62,0.05)",border:"1px solid rgba(44,74,62,0.18)",padding:"8px 11px",color:TEXT,fontFamily:"'DM Sans',sans-serif",fontSize:12,outline:"none",resize:"vertical",lineHeight:1.5}} />
            </div>
            <div>
              <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:"rgba(44,74,62,0.4)",letterSpacing:"0.09em",textTransform:"uppercase",marginBottom:8}}>Scale of Change (1 = minor, 5 = transformational)</div>
              <div style={{display:"flex",alignItems:"center",gap:14}}>
      ...
      {view === "adkar" && (
        <div>
          ...
          {(() => {
            ...
            return (
              <Card style={{padding:24}}>
                <div style={{fontFamily:"'Playfair Display',serif",fontSize:18,fontWeight:700,color:TEXT,marginBottom:4}}>{el.label}</div>
                <div style={{fontSize:12,color:"rgba(44,74,62,0.5)",lineHeight:1.6,marginBottom:14}}>{el.desc}</div>
                <div style={{background:"rgba(44,74,62,0.06)",borderLeft:`3px solid ${el.color}`,padding:"10px 14px",marginBottom:18,fontSize:11,color:"rgba(44,74,62,0.45)",lineHeight:1.6,fontStyle:"italic"}}>{el.barrier}</div>
                  ...
                  {[1,2,3,4,5].map(n => {
                    ...
                    return (
                      <div key={n} onClick={() => setAdkarScores(p=>({...p,[el.id]:n}))} style={{padding:"12px 8px",cursor:"pointer",border:`2px solid ${isOn?el.color:"transparent"}`,background:`${el.color}${isOn?"22":"0a"}`,textAlign:"center",transition:"all 0.12s"}}>
                        ...
                        <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:isOn?el.color:"rgba(44,74,62,0.3)",textTransform:"uppercase",letterSpacing:"0.07em",marginTop:4}}>{labels[n]}</div>
                      </div>
                    );
                  })}
                </div>
      ...
      {view === "resistance" && (
        <div>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <span style={{fontFamily:"'DM Mono',monospace",fontSize:10,color:"rgba(44,74,62,0.35)"}}>{resistanceFactors.length} factor{resistanceFactors.length!==1?"s":""} identified</span>
        ...
        <div>
              ...
              {ADKAR_ELEMENTS.map(el => {
                ...
                return (
                  <div key={el.id} style={{
                    ...
                    <div style={{fontFamily:"'DM Mono',monospace",fontSize:8,color:"rgba(44,74,62,0.3)",marginTop:2}}>/5</div>
                  </div>
                );
              })}
            </div>
          ...
          <Card style={{padding:24,marginBottom:12}}>
                ...
                <div key={el.id} style={{marginBottom:16,paddingBottom:16,borderBottom:"1px solid rgba(247,244,239,0.05)"}}>
                  ...
                  {interventions[el.id].slice(0, s===1?5:s===2?4:3).map((action,i) => (
                    <div key={i} style={{display:"flex",gap:8,alignItems:"flex-start",marginBottom:5}}>
                      ...
                      <span style={{fontSize:12,color:"rgba(44,74,62,0.55)",lineHeight:1.6}}>{action}</span>
                    </div>
                  ))}
                </div>
              );
            ...
            {ADKAR_ELEMENTS.every(el => (adkarScores[el.id]||0) >= 4) && (
              <div style={{textAlign:"center",padding:24,color:"rgba(44,74,62,0.4)",fontStyle:"italic",fontSize:13}}>Strong ADKAR scores across all elements. Focus on reinforcement to sustain adoption.</div>
            )}
          </Card>
              ...
              {resistanceFactors.map(id => {
                ...
                return (
                  <div key={id} style={{borderLeft:`2px solid ${riskCol}`,paddingLeft:16,marginBottom:14}}>
                    ...
                    <div style={{fontSize:12,color:"rgba(44,74,62,0.55)",lineHeight:1.6}}>{responses[id]}</div>
                  </div>
                );
              })}
            </Card>
  ...
  return(<div style={{minHeight:"100vh",background:BG,display:"flex",flexDirection:"column"}}>
    ...
    <header style={{borderBottom:"1px solid rgba(44,74,62,0.12)",padding:"14px 24px",display:"flex",justifyContent:"space-between",alignItems:"center",background:BG,boxShadow:"0 1px 0 rgba(44,74,62,0.12)",position:"sticky",top:0,zIndex:200,flexShrink:0}}>
      <div style={{display:"flex",alignItems:"center",gap:14}}>
        ...
        <div>
          <div style={{...serif,fontSize:13,fontWeight:700,color:TEXT,lineHeight:1.1}}>STRATEGY <em style={{fontWeight:400,color:GOLD}}>Made Real</em></div>
          <div style={{...mono,fontSize:7,color:"rgba(44,74,62,0.3)",letterSpacing:"0.14em",textTransform:"uppercase",marginTop:2}}>Beyond theory. Into delivery. Powered by people.</div>
        </div>
      </div>
      <div style={{...mono,fontSize:9,color:"rgba(44,74,62,0.3)",letterSpacing:"0.12em",textAlign:"right",lineHeight:1.9,textTransform:"uppercase"}}>Strategy Execution Suite</div>
    </header>
    ...
    <div style={{display:"flex",flex:1,overflow:"hidden"}}>
      ...
      <div style={{width:240,flexShrink:0,borderRight:"1px solid rgba(44,74,62,0.12)",background:"rgba(44,74,62,0.04)",overflowY:"auto",position:"sticky",top:57,height:"calc(100vh - 57px)"}}>
        ...
        <div style={{padding:"16px 18px",borderTop:"1px solid rgba(44,74,62,0.1)",marginTop:"auto"}}>
          <div style={{...mono,fontSize:8,color:"rgba(44,74,62,0.18)",letterSpacing:"0.08em",lineHeight:1.8}}>Strategy Made Real<br/>Execution Suite<br/>Eight Tools</div>
        </div>
      </div>
      ...
      <div style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column"}}>
        ...
        <div style={{padding:"24px 32px 20px",borderBottom:"1px solid rgba(44,74,62,0.1)",position:"relative",overflow:"hidden",flexShrink:0}}>
          ...
          <div style={{...serif,fontSize:"clamp(18px,2.5vw,30px)",fontWeight:700,color:TEXT,lineHeight:1.15}}>{active.label}</div>
          <div style={{fontSize:13,color:"rgba(44,74,62,0.45)",marginTop:4}}>{active.sub}</div>
        </div>
