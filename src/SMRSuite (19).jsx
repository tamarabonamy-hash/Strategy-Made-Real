...
body,#root{background:#2C4A3E;color:#1C2B25;font-family:'DM Sans',sans-serif;min-height:100vh;}
::-webkit-scrollbar{width:5px;}
::-webkit-scrollbar-track{background:#2C4A3E;}
::-webkit-scrollbar-thumb{background:#5A7A6E;border-radius:3px;}
...
function Tool4RoleAnalyser(){
  ...
  async function analyse(){
    ...
    try{
      ...
      const response=await fetch("/api/analyse",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-5-20251001",max_tokens:4000,messages:[{role:"user",content:prompt}]})});
      const data=await response.json();
      const text=data.content?.[0]?.text||"";
      const parsed=JSON.parse(text.replace(/```json|```/g,"").trim());
      setResult(parsed);setOpen({"1":true});
    }catch(e){const errMsg = e?.message||"Unknown error";
      setError("API error: " + errMsg + ". Check that your Vercel API key is set correctly.");}
    setLoading(false);
  }
...
export default function SMRSuite(){
  ...
  return(<div style={{minHeight:"100vh",background:G,display:"flex",flexDirection:"column"}}>
    <style>{CSS}</style>

    {/* ── Header ── */}
    <header style={{background:S1,borderBottom:`1px solid ${BDR}`,padding:"14px 28px",display:"flex",justifyContent:"space-between",alignItems:"center",position:"sticky",top:0,zIndex:200,flexShrink:0}}>
      <div style={{display:"flex",alignItems:"center",gap:14}}>
        <svg width="32" height="32" viewBox="0 0 120 120" fill="none">
          <ellipse cx="62" cy="60" rx="48" ry="52" stroke={G} strokeWidth="1.5" strokeOpacity="0.4"/>
          <ellipse cx="68" cy="60" rx="32" ry="52" stroke={G} strokeWidth="1.5" strokeOpacity="0.4"/>
          <ellipse cx="62" cy="60" rx="18" ry="20" stroke={G} strokeWidth="1.5" strokeOpacity="0.4"/>
          <circle cx="74" cy="60" r="10" fill={GOLD}/>
          <circle cx="74" cy="60" r="5" fill={G} fillOpacity="0.2"/>
          <line x1="38" y1="28" x2="44" y2="90" stroke={G} strokeWidth="1.2" strokeOpacity="0.35"/>
        </svg>
        <div>
          ...
          <div style={{...mono,fontSize:8,color:T4,letterSpacing:"0.12em",textTransform:"uppercase",marginTop:2}}>Beyond theory. Into delivery. Powered by people.</div>
        </div>
      </div>
      <div style={{...mono,fontSize:9,color:T4,letterSpacing:"0.1em",textTransform:"uppercase",textAlign:"right",lineHeight:1.8}}>Strategy Execution Suite</div>
    </header>
    ...
    <div style={{display:"flex",flex:1,overflow:"hidden"}}>
      ...
      {/* ── Sidebar ── */}
      <nav style={{width:236,flexShrink:0,background:S2,borderRight:`1px solid ${BDR}`,overflowY:"auto",position:"sticky",top:57,height:"calc(100vh - 57px)",display:"flex",flexDirection:"column"}}>
        <div style={{padding:"10px 0",flex:1}}>
          {TOOLS.map(t=>(
            <button key={t.id} onClick={()=>setActiveTool(t.id)} style={{width:"100%",background:activeTool===t.id?S1:"transparent",border:"none",borderLeft:activeTool===t.id?`3px solid ${GOLD}`:`3px solid transparent`,padding:"11px 16px",cursor:"pointer",textAlign:"left",transition:"background 0.15s",display:"flex",gap:10,alignItems:"center"}}>
              <div style={{minWidth:0}}>
                <div style={{...mono,fontSize:9,color:activeTool===t.id?GOLD:T4,letterSpacing:"0.08em",textTransform:"uppercase",marginBottom:1}}>{t.num}</div>
                <div style={{...serif,fontSize:12,fontWeight:700,color:activeTool===t.id?T1:T3,lineHeight:1.25,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{t.label}</div>
                <div style={{fontSize:10,color:activeTool===t.id?T3:T5,marginTop:1,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{t.sub}</div>
              </div>
            </button>
          ))}
        </div>
        <div style={{padding:"14px 16px",borderTop:`1px solid ${BDR}`}}>
          <div style={{...mono,fontSize:8,color:T5,letterSpacing:"0.08em",lineHeight:1.9}}>Strategy Made Real<br/>Eight Tools · One Suite</div>
        </div>
      </nav>
      ...
      <div style={{flex:1,overflowY:"auto",background:S1}}>
        ...
        {/* Tool hero */}
        <div style={{padding:"22px 32px 18px",borderBottom:`1px solid ${BDR}`,background:S1,flexShrink:0}}>
